// supabase/functions/payments-webhook-mollie/index.ts
...
serve(async (req) => {
  try {
    ...
    if (updateError) console.error("Error updating payment in DB:", updateError);

    // If paid, create invoice and send receipt email
    if (newMollieStatus === 'paid' && orderId) {
      ...
      // 3. Send email receipt via email-send function
