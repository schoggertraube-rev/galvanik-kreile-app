-- F0-04: erlaube Vorwaertsreferenzen in Funktions-Bodies (pg_dump-Standard)
...
CREATE POLICY scan_objects_insert_authenticated ON storage.objects AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (((bucket_id = 'scans'::text) AND (EXISTS ( SELECT 1 FROM public.app_users au WHERE ((au.id = auth.uid()) AND (au.active IS TRUE) AND (au.tenant_id = (storage.foldername(objects.name))[1]) AND ((au.role)::text = ANY ((ARRAY['werkstatt'::character varying, 'meister'::character varying, 'buero'::character varying, 'admin'::character varying])::text[])))))));

drop policy if exists "scan_objects_select_authenticated" on storage.objects;
CREATE POLICY scan_objects_select_authenticated ON storage.objects AS PERMISSIVE FOR SELECT TO authenticated USING (((bucket_id = 'scans'::text) AND (EXISTS ( SELECT 1 FROM public.app_users au WHERE ((au.id = auth.uid()) AND (au.active IS TRUE) AND (au.tenant_id = (storage.foldername(objects.name))[1]))))));
