...
-- Existing production tables predate the tenant columns that fresh CREATE
-- statements already contain. Establish the additive tenant identity before
-- any index, quarantine, or relationship below reads it. The later 00575
-- migration remains the authoritative relationship reconciliation.
DO $marketing_tenant_expand$
DECLARE
  relation_name text;
...
BEGIN
  ...
  JOIN pg_type type_record ON type_record.oid = attribute_record.atttypid
  WHERE attribute_record.attrelid = 'public.ausgangsrechnung'::regclass
    AND attribute_record.attname = 'lead_id'
    AND NOT attribute_record.attisdropped;

  IF lead_type NOT IN ('uuid', 'text') THEN
    RAISE EXCEPTION USING
      ERRCODE = '42804',
      MESSAGE = 'MARKETING_INVOICE_LEAD_TYPE_RECONCILIATION_REQUIRED';
  END IF;

  IF lead_type = 'uuid' THEN
    IF EXISTS (
      SELECT 1
      FROM public.ausgangsrechnung invoice
      LEFT JOIN public.inquiries inquiry
        ON inquiry.tenant_id = invoice.tenant_id
       AND inquiry.id = invoice.lead_id::text
      WHERE invoice.lead_id IS NOT NULL
        AND inquiry.id IS NULL
    ) THEN
      RAISE EXCEPTION USING
        ERRCODE = '23503',
        MESSAGE = 'MARKETING_INVOICE_LEAD_RECONCILIATION_REQUIRED';
    END IF;
