...
function getLegacySurface(
  part: WarendurchlaufOrder["parts"][number] | undefined,
) {
  if (
    part &&
    typeof part === "object" &&
...
export function OffeneAuftraegeKachel({
  ...
  data: WarendurchlaufKpiData | null;
}) {
        ...
        <div className="flex flex-col gap-3">
          {sortedRecent.length > 0 ? (
            sortedRecent.map((o) => {
              const partsStr =
                o.parts.length > 0
                  ? o.parts.map((p) => p.name).join(", ")
                  : "Artikel nicht hinterlegt";
              const surfaceStr =
                getLegacySurface(o.parts[0]) || "Oberfläche nicht hinterlegt";
              ...
              return (
                <OrderQueueRow
                  ...
                  order={{
                    ...
                    title: partsStr,
                    detail: surfaceStr,
                    station: o.status || "Offen",
                    dueLabel:
                      o.dueValue || o.dueLabel || "Termin nicht erfasst",
                    risk: o.risk,
                  }}
                  onOpen={openOrder}
                />
            ...
            <div className="p-8 text-center bg-white border border-dashed border-neutral-gray-300 rounded-2xl flex flex-col items-center justify-center">
              ...
              <p className="text-text-muted text-sm">
                Aktuell befinden sich keine Aufträge im Durchlauf.
              </p>
            </div>
          )}
        </div>
