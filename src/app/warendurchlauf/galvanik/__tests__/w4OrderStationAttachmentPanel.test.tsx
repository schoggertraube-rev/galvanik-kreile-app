...
vi.mock(
  "@/app/warendurchlauf/galvanik/GalvanikHandoffAttachmentAppAdapter",
  async () => {
    const orderModule = await vi.importActual<
      typeof import("@/modules/orders/public")
    ...
    return {
      GalvanikHandoffAttachmentAppAdapter: (
        ...
        <orderModule.OrderStationAttachmentPanel
          {...props}
          ports={{
            ...
            uploadSigned: ({ bucketId, path, token, bytes, contentType }) => {
              ...
              return ports.upload(path, token, bytes, {
                contentType,
                upsert: false,
              });
            },
...
vi.mock("@/modules/orders/public", async () => ({
  ...(await vi.importActual<typeof import("@/modules/orders/public")>(
    "@/modules/orders/public",
  )),
  OrderQueueRow: ({
    order,
...
import {
  OrderCardView,
  OrderStationAttachmentPanel as ModuleGalvanikHandoffAttachmentPanel,
  type OrderStationAttachmentPanelProps as GalvanikHandoffAttachmentPanelProps,
...
function envelope(
  ...
  return {
    code: "OK" as const,
    data: { receipts, evidenceRecords, canOperate, currentActorId },
  };
}
...
async function chooseFile() {
  const input = screen.getByLabelText(/Neues Original|Datei erneut wählen/i, {
    selector: "input",
  });
...
beforeEach(() => {
  ...
  ports.upload.mockResolvedValue({
...
describe("W4 Galvanik handoff attachment panel", () => {
  it("renders zero/many fail-closed and readonly metadata without mutation controls", async () => {
    ...
    expect(screen.getByRole("alert")).toHaveTextContent(
      "Kein eindeutig zugeordnetes Teil",
    );
    expect(screen.getByLabelText("Galvanik-Übergabeoriginal")).toHaveAttribute(
      "aria-busy",
      "false",
    );
    expect(
      zero.container.querySelector("input[type=file]"),
    ...
    const many = render(
      <GalvanikHandoffAttachmentPanel
        ...
        expectedVersion={2}
        items={[
          { id: ITEM_ID, name: "Teil A" },
          { id: "item-b", name: "Teil B" },
        ]}
      />,
    );
    ...
    await waitFor(() =>
      expect(ports.getAttachments).toHaveBeenCalledWith({
        orderId: ORDER_ID,
        itemId: ITEM_ID,
      }),
    );
    ...
    ports.getAttachments.mockResolvedValueOnce(
      envelope(
        [
          receipt(),
          finalized({
  ...
  it("renders verified no-extraction truth and read-only legacy confidence with polymorphic targets", async () => {
    ...
    expect(
      screen.getByText(/Extraktion übernommen: Lieferschein · Konfidenz 91 %/),
    ).toBeInTheDocument();
  ...
  it("uploads the exact hashed bytes and shows success only after FINALIZED fresh readback", async () => {
    ...
    const finalize = deferred<{
      code: "OK";
      data: { receipt: ReturnType<typeof finalized>; replayed: false };
    }>();
    ...
    expect(ports.upload.mock.calls[0][0]).toBe(
    ...
    expect(Array.from(uploadBody as Uint8Array)).toEqual(
      Array.from(FILE_BYTES),
    );
    expect(ports.upload.mock.calls[0][3]).toEqual({
    ...
    expect(screen.getByRole("status")).toHaveTextContent(
      "Gespeichertes Original wird geprüft",
    );
    expect(
      screen.queryByText(/Original sicher gespeichert/),
    ).not.toBeInTheDocument();
  ...
  it.each([
    ...
    "rejects a reserve response with a wrong %s before upload",
    async (_label, wrongBinding) => {
      const pending = receipt({
        reservationId: NEW_RESERVATION,
        clientRequestId: NEW_REQUEST,
        ...wrongBinding,
  ...
  it("rejects an exact-replay response that switches the reservation id", async () => {
    ...
    expect(ports.reserve).toHaveBeenCalledWith(
      expect.objectContaining({ clientRequestId: REQUEST }),
    );
    expect(ports.upload).not.toHaveBeenCalled();
    ...
    async (failure) => {
      ...
      ports.upload.mockResolvedValueOnce(
        failure === "upload error"
          ? {
              data: null,
              error: { message: "provider detail must stay irrelevant" },
            }
          : {
              data: {
                path:
                  failure === "upload response path"
  ...
  it.each([
    ...
    "requires fresh readback equality for %s",
    async (field, wrongValue) => {
      ...
      ports.finalize.mockResolvedValueOnce({
        code: "OK",
        data: { receipt: done, replayed: false },
      });
      render(
        <GalvanikHandoffAttachmentPanel
          orderId={ORDER_ID}
      ...
      await chooseFile();
      await waitFor(() =>
        expect(ports.getAttachments).toHaveBeenCalledTimes(3),
      );
      expect(
        screen.queryByText(/Original sicher gespeichert/),
      ).not.toBeInTheDocument();
  ...
  it("routes same-intent retry to the newest owned PENDING and never a coworker row", async () => {
    ...
    await waitFor(() => expect(ports.reserve).toHaveBeenCalledTimes(1));
    expect(ports.finalize).toHaveBeenCalledWith({
      reservationId: newer.reservationId,
    });
    expect(ports.reserve).toHaveBeenCalledWith(
  ...
  it("replays one actor-owned PENDING intent without minting a new client request", async () => {
    ...
    ports.reserve.mockResolvedValueOnce({
      ...
      data: {
        receipt: pending,
        upload: {
          path: `order-station-evidence/v1/${RESERVATION}.png`,
          token: "replay-token",
        },
        replayed: true,
      },
  ...
  it("uses a keyed two-selection restart and rechecks an expired reservation before any new path", async () => {
    ...
    await chooseFile();
    expect(
      await screen.findByText("Erneute Prüfung erforderlich"),
    ).toBeInTheDocument();
  ...
  it("creates a new immutable path only after an expired replay is rechecked and still not ready", async () => {
    ...
    await chooseFile();
    expect(await screen.findByText("Bestätigt")).toBeInTheDocument();
    expect(ports.finalize).toHaveBeenNthCalledWith(2, {
      reservationId: RESERVATION,
    });
    expect(ports.reserve).toHaveBeenNthCalledWith(
      2,
      expect.objectContaining({
        clientRequestId: NEW_REQUEST,
      }),
  ...
  it.each(["UPLOAD_MISMATCH", "UPLOAD_OUTSIDE_WINDOW"] as const)(
    ...
    async (reason) => {
      ...
      await chooseFile();
      expect(
        await screen.findByText("Neue Reservierung erforderlich"),
      ).toBeInTheDocument();
      expect(ports.reserve).not.toHaveBeenCalled();
      await chooseFile();
      await waitFor(() => expect(ports.reserve).toHaveBeenCalledTimes(1));
      expect(ports.reserve).toHaveBeenCalledWith(
        expect.objectContaining({ clientRequestId: NEW_REQUEST }),
      );
    },
  );
  ...
  it.each([
    ...
    await screen.findByText("Noch kein Übergabeoriginal erfasst.");
    const input = screen.getByLabelText(/Neues Original/i, {
      selector: "input",
    });
  ...
  it.each([
    [
      ...
      { orderId: "order-b", itemId: ITEM_ID },
    ],
    [
      "version",
      {
    ...
    [
      "item",
      {
        orderId: ORDER_ID,
        expectedVersion: 2,
        items: [{ id: "item-b", name: "Teil B" }],
      },
      { orderId: ORDER_ID, itemId: "item-b" },
    ],
  ] as const)(
    "invalidates a deferred workflow when only the %s scope changes",
    async (_label, nextProps, expectedRead) => {
      const reserve = deferred<ReturnType<typeof reserveOk>>();
      ports.reserve.mockReturnValueOnce(reserve.promise);
      const view = render(
        <GalvanikHandoffAttachmentPanel
  ...
  it("stops after a deferred hash when unmounted", async () => {
    ...
    await waitFor(() =>
      expect(customFile.arrayBuffer).toHaveBeenCalledTimes(1),
    );
    view.unmount();
  ...
  it("binds the signed original exactly and expires it from request start", async () => {
    ...
    expect(
      screen.queryByRole("link", { name: "Privates Original jetzt öffnen" }),
    ).not.toBeInTheDocument();
    ...
    const link = screen.getByRole("link", {
      name: "Privates Original jetzt öffnen",
    });
  ...
  it.each([
    [
      "wrong origin",
      {
        downloadUrl: `https://evil.example/storage/v1/object/sign/item-photos/order-station-evidence/v1/${RESERVATION}.png?token=x&download=galvanik-uebergabe-original.png`,
      },
    ],
    [
      "wrong path",
      {
        downloadUrl: `http://127.0.0.1:54321/storage/v1/object/sign/item-photos/order-station-evidence/v1/${NEW_RESERVATION}.png?token=x&download=galvanik-uebergabe-original.png`,
      },
    ],
    [
      "extra query",
      {
        downloadUrl: `http://127.0.0.1:54321/storage/v1/object/sign/item-photos/order-station-evidence/v1/${RESERVATION}.png?token=x&download=galvanik-uebergabe-original.png&extra=1`,
      },
    ],
    [
      "duplicate token",
      {
        downloadUrl: `http://127.0.0.1:54321/storage/v1/object/sign/item-photos/order-station-evidence/v1/${RESERVATION}.png?token=x&token=y&download=galvanik-uebergabe-original.png`,
      },
    ],
    [
      "wrong download name",
      {
        downloadUrl: `http://127.0.0.1:54321/storage/v1/object/sign/item-photos/order-station-evidence/v1/${RESERVATION}.png?token=x&download=wrong.png`,
      },
    ],
  ...
  it("fails closed for wrong metadata graph rows before any original action", async () => {
    ports.getAttachments.mockResolvedValueOnce(
      envelope([finalized({ orderId: "other-order" })]),
    );
    render(
      <GalvanikHandoffAttachmentPanel
        orderId={ORDER_ID}
    ...
    expect(await screen.findByRole("alert")).toHaveTextContent(
      "nicht exakt an Auftrag und Teil",
    );
    expect(
      screen.queryByRole("button", { name: "Original freigeben" }),
    ).not.toBeInTheDocument();
...
describe("W4 V8 order-card placement", () => {
  it("mounts exactly one supported panel inside the canonical V8 card", async () => {
    ports.getAttachments.mockResolvedValue(envelope([]));
    ports.reserve.mockResolvedValueOnce({
      code: "CONFLICT",
      reason: "ORDER_CHANGED",
      ...
      <OrderCardView
        state={{
          ...
          card: {
            ...
            status: "galvanik",
            dueAt: null,
            intakeAt: "2026-09-15T08:00:00.000Z",
            assignedTo: null,
            items: [
              {
                id: ITEM_ID,
            ...
            payment: { kind: "restricted", message: "Geschützt" },
          },
        }}
        actions={{
          ...
          invoice: { visible: false, enabled: false, reason: null },
          payment: { visible: false, enabled: false, reason: null },
          goodsOut: { visible: false, enabled: false, reason: null },
          feedback: { kind: "idle", message: "" },
          onHandoff: vi.fn(),
          onFreeze: vi.fn(),
          onIssueInvoice: vi.fn(),
          onConfirmPayment: vi.fn(),
          onRecordGoodsOut: vi.fn(),
          onReload: vi.fn(),
        }}
        attachmentPanel={
          <GalvanikHandoffAttachmentPanel
    ...
    expect(screen.getAllByLabelText("Galvanik-Übergabeoriginal")).toHaveLength(
      1,
    );
    await waitFor(() =>
      expect(ports.getAttachments).toHaveBeenCalledWith({
    ...
    expect(
      await screen.findByText("Neues Original für Teil A"),
    ).toBeInTheDocument();
    await chooseFile();
    await waitFor(() =>
      expect(ports.reserve).toHaveBeenCalledWith(
