...
vi.mock("@/modules/orders/public", () => ({
  isOrderStationForwardRole: (role: string) =>
    ["buero", "werkstatt", "meister", "admin"].includes(role),
  OrderQueueRow: ({ order }: { order: { orderNumber: string } }) => (
    <div>{order.orderNumber}</div>
  ),
}));
vi.mock("@/lib/overlayStore", () => ({
  useOverlayStore: (
    selector?: (state: { openOrder: typeof ports.openOrder }) => unknown,
  ) =>
    selector
...
beforeEach(() => {
  ...
  ports.getWarendurchlaufKPIs.mockResolvedValue({
    ok: true,
...
describe("W2C-B2M5J unavailable UI", () => {
  it("renders only the truthful station-loading status while the initial station read is pending", async () => {
    ...
    const deferredStationOrders = new Promise<{ ok: true; data: [] }>(
      (resolvePromise) => {
        resolveStationOrders = resolvePromise;
      },
    );
    ports.getWareneingangOrdersAction.mockReturnValueOnce(
      deferredStationOrders,
    );
    ...
    expect(
      screen
        .getByText("Stationsliste wird geladen.", { exact: true })
        .closest('[role="status"]'),
    ).not.toBeNull();
  ...
  it("renders authorized real werkstatt data with real risk-derived Heute-sichern cards and opens only the existing order", async () => {
    const wareneingang = order(
      "we-1",
      "WE-001",
      "Wareneingang Sentinel",
      "wareneingang",
      "orange",
    );
    const galvanik = order(
      "ga-1",
      "GA-001",
      "Galvanik Sentinel",
      "fertig",
    ...
    ports.getWarendurchlaufKPIs.mockResolvedValueOnce({
      ok: true,
    ...
    expect(
      screen.getByRole("heading", { level: 1, name: "Werkstatt" }),
    ).toBeInTheDocument();
    ...
    expect(screen.getByTestId("werkstatt-held-we-1")).toHaveTextContent("Wareneingang Sentinel");
    expect(screen.getByTestId("werkstatt-held-ga-1")).toHaveTextContent("Galvanik Sentinel");
    ...
    const wipTile = screen.getByRole("button", {
      name: /In Arbeit \(Galvanik\)/,
    });
    ...
    const wareneingangOrder = screen.getByRole("button", {
      name: /Auftrag WE-001/,
    });
    const galvanikOrder = screen.getByRole("button", {
      name: /Auftrag GA-001/,
    });
  ...
  it("suggests a bundle only when surfaceRequested repeats and treats it as a filter, never a mutation", async () => {
    ...
    const filterButton = within(bundle).getByRole("button", {
      name: "Diese Aufträge zeigen",
    });
    ...
    expect(within(heldList).getAllByTestId(/^werkstatt-held-/)).toHaveLength(2);
  ...
  it("opens the real-order picker in Galvanik-first order and selects the exact existing order id", async () => {
    const wareneingang = order(
      "we-picker",
      "WE-PICKER",
      "Wareneingang Picker Sentinel",
      "wareneingang",
    );
    const galvanik = order(
      "ga-picker",
      "GA-PICKER",
      "Galvanik Picker Sentinel",
      "fertig",
    );
    ports.getWareneingangOrdersAction.mockResolvedValueOnce({
      ok: true,
    ...
    expect(
      within(dialog).getAllByText("GA-PICKER", { exact: true }),
    ).toHaveLength(1);
    expect(
      within(dialog).getAllByText("WE-PICKER", { exact: true }),
    ).toHaveLength(1);
  ...
  it("also opens the same real-order picker from Mehrarbeit and Fertig melden, never faking a mutation", async () => {
    ...
    expect(
      screen.getByRole("dialog", { name: "Auftrag öffnen" }),
    ).toBeInTheDocument();
  ...
  it("uses the existing fail-closed scan capture path from the Mock picker", async () => {
    ports.getWareneingangOrdersAction.mockResolvedValueOnce({
      ok: true,
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Auftrag öffnen" }),
    );
    fireEvent.click(screen.getByRole("button", { name: "Nummer, Kunde oder Scan …" }));
  ...
  it("opens Ware raus with only canonical fertig candidates and selects through the injected app port", async () => {
    ...
    expect(
      within(dialog).getByTestId("goods-out-picker-order-fi-out"),
    ).toBeVisible();
    expect(
      within(dialog).queryByText("WE-OUT", { exact: true }),
    ).not.toBeInTheDocument();
    expect(
      within(dialog).queryByText("GA-OUT", { exact: true }),
    ).not.toBeInTheDocument();
    expect(
      within(dialog).queryByRole("button", { name: "Auftrag scannen" }),
    ).not.toBeInTheDocument();
    ...
    expect(
      screen.queryByRole("dialog", { name: "Ware raus" }),
    ).not.toBeInTheDocument();
  });
  ...
  it("shows an honest Ware-raus empty picker when real data contains no fertig order", async () => {
    ...
    const dialog = screen.getByRole("dialog", { name: "Ware raus" });
    expect(
      within(dialog).getByTestId("goods-out-picker-empty"),
    ).toHaveTextContent("Keine fertig gemeldete Ware zur Ausgabe vorhanden.");
    expect(
      within(dialog).queryAllByTestId(/^goods-out-picker-order-/),
    ).toHaveLength(0);
    expect(ports.openOrder).not.toHaveBeenCalled();
  });
  ...
  it("traps Tab focus, closes the real-order picker explicitly or with Escape, and returns focus", async () => {
    ...
    const dialog = screen.getByRole("dialog", { name: "Auftrag öffnen" });
    const lastPickerOrder = within(dialog).getByTestId(
      "order-picker-order-we-focus-last",
    );
  ...
  it("renders a truthful root empty state only after both station ports succeed empty", async () => {
    ...
    expect(screen.getByTestId("werkstatt-status")).toHaveTextContent("Heute keine Termine.");
    expect(screen.queryByRole("alert")).not.toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: "Auftrag öffnen" }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole("dialog", { name: "Auftrag öffnen" }),
    ).not.toBeInTheDocument();
    ...
    expect(screen.queryByRole("button", { name: "Ware raus" })).not.toBeInTheDocument();
  });

  it("denies an excluded root role before either station action is invoked", async () => {
    ports.resolveAuthorization.mockResolvedValueOnce(
      allowedAuthorization("developer"),
    );
    const { default: WarendurchlaufIndex } = await import("../page");
    render(await WarendurchlaufIndex());
  ...
  it("suppresses a successful Wareneingang payload when Galvanik denies the atomic root read", async () => {
    const sentinel = order(
      "we-secret",
      "WE-SECRET",
      "Wareneingang Teilresultat Sentinel",
      "wareneingang",
    );
    ports.getWareneingangOrdersAction.mockResolvedValueOnce({
      ok: true,
    ...
    expect(screen.getByText("Zugriff nicht erlaubt.")).toBeInTheDocument();
    expect(
      screen.queryByText("Wareneingang Teilresultat Sentinel"),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByTestId("werkstatt-held-we-secret"),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: "Auftrag öffnen" }),
    ).not.toBeInTheDocument();
  ...
  it("suppresses a successful Galvanik payload when Wareneingang fails the atomic root read", async () => {
    const sentinel = order(
      "ga-secret",
      "GA-SECRET",
      "Galvanik Teilresultat Sentinel",
      "fertig",
    );
    ports.getWareneingangOrdersAction.mockResolvedValueOnce({
      ok: false,
    ...
    expect(
      screen.getByText("Werkstattdaten konnten nicht sicher geladen werden."),
    ).toBeInTheDocument();
    expect(
      screen.queryByText("Galvanik Teilresultat Sentinel"),
    ).not.toBeInTheDocument();
    expect(
  ...
  it("suppresses both station payloads when the canonical KPI read fails closed", async () => {
    ports.getWareneingangOrdersAction.mockResolvedValueOnce({
      ok: true,
      data: [
        order("we-kpi", "WE-KPI", "Wareneingang KPI Sentinel", "wareneingang"),
      ],
    });
    ports.getGalvanikOrdersAction.mockResolvedValueOnce({
      ok: true,
  ...
  it.each([
    [
      ...
      order(
        "duplicate-id",
        "WE-DUP-ID",
        "Duplicate ID Wareneingang Sentinel",
        "wareneingang",
      ),
      order(
        "duplicate-id",
        "GA-DUP-ID",
        "Duplicate ID Galvanik Sentinel",
        "galvanik",
      ),
    ],
    [
      ...
      order(
        "we-duplicate-number",
        "DUP-NUMBER",
        "Duplicate Number Wareneingang Sentinel",
        "wareneingang",
      ),
      order(
        "ga-duplicate-number",
        "DUP-NUMBER",
        "Duplicate Number Galvanik Sentinel",
        "galvanik",
      ),
    ],
  ])(
    "renders a data-free conflict for a duplicate canonical %s",
    async (_field, wareneingang, galvanik) => {
      ports.resolveAuthorization.mockResolvedValueOnce(
        allowedAuthorization("buero"),
      );
      ports.getWareneingangOrdersAction.mockResolvedValueOnce({
      ...
      expect(screen.getByRole("alert")).toHaveTextContent("Werkstattdaten enthalten widersprüchliche Auftragskennungen.");
      expect(
        screen.getByText(
          "Werkstattdaten enthalten widersprüchliche Auftragskennungen.",
        ),
      ).toBeVisible();
      ...
      expect(
        screen.queryByTestId(`werkstatt-held-${galvanik.id}`),
      ).not.toBeInTheDocument();
      expect(
        screen.queryByRole("button", { name: "Neuer Eingang" }),
      ).not.toBeInTheDocument();
  ...
  it("renders an honest loading state without fake operational data", async () => {
    ...
    expect(
      screen.getByRole("region", { name: "Werkstatt" }),
    ).toHaveAttribute("aria-busy", "true");
    expect(
      screen.getByRole("heading", { level: 1, name: "Werkstatt" }),
    ).toBeInTheDocument();
    expect(
      screen.queryByText(/WE-001|GA-001|Noch keine Daten erfasst/),
    ).not.toBeInTheDocument();
  });

  it("source-locks the Phillip-Mock hierarchy, global Mock CSS, and real ports", () => {
    ...
    const cssSource = readFileSync(
      resolve(
        process.cwd(),
        "src/styles/mock/mock-kreile-phillip-home.css",
      ),
      "utf8",
    );
    const layoutSource = readFileSync(resolve(process.cwd(), "src/app/layout.tsx"), "utf8");
    const compatSource = readFileSync(resolve(process.cwd(), "src/styles/mock/mock-kreile-compat.css"), "utf8");
    const adapterSource = readFileSync(
      resolve(process.cwd(), "src/app/warendurchlauf/WerkstattAppAdapter.tsx"),
      "utf8",
    );
    const routeSource = readFileSync(
      resolve(process.cwd(), "src/app/warendurchlauf/page.tsx"),
      "utf8",
    );
    ...
    expect(clientSource).toContain("Auftrag öffnen");
    ...
    expect(clientSource).toContain('className="mock-kreile-phillip-home"');
    expect(clientSource).not.toContain("WerkstattView.module.css");
    expect(cssSource).toContain("AUTOMATISCH aus dem Owner-Mock erzeugt");
    expect(cssSource).toContain(".mock-kreile-phillip-home .actionbar");
    expect(cssSource).toContain("@media (prefers-reduced-motion:reduce)");
    expect(layoutSource).toContain('mock-kreile-phillip-home.css');
    expect(compatSource).toContain(".content > .mock-kreile-phillip-home");
    expect(compatSource).toContain(".tcontent > .mock-kreile-phillip-home");
    ...
    expect(routeSource).toContain(
      "galvanikResult.data.map(toWerkstattSurfaceOrder)",
    );
  ...
  it("renders station denial without confirmed empty-station success while KPI tiles remain unavailable", async () => {
    ports.getWareneingangOrdersAction.mockResolvedValueOnce({
      ok: false,
      error: "NOT_AVAILABLE",
      message: stationDenial,
    } as never);
    const { default: WareneingangPage } = await import("../wareneingang/page");
    render(<WareneingangPage />);

    await waitFor(() =>
      expect(screen.getByText(stationDenial)).toBeInTheDocument(),
    );
    expect(
      screen.getByText(stationDenial).closest('[role="status"]'),
    ).not.toBeNull();
    ...
    expect(
      screen.queryByText("Termintreue Durchlaufzeit Engpass Offene Aufträge"),
    ).not.toBeInTheDocument();
  ...
  it("renders the stable station denial when the station read throws", async () => {
    ports.getWareneingangOrdersAction.mockRejectedValueOnce(
    ...
    await waitFor(() =>
      expect(screen.getByText(stationThrowDenial)).toBeInTheDocument(),
    );
    expect(
      screen.getByText(stationThrowDenial).closest('[role="status"]'),
    ).not.toBeNull();
    expect(screen.queryByText("0")).not.toBeInTheDocument();
    expect(
      screen.queryByText("Noch keine Daten erfasst."),
  ...
  it("keeps a successful empty station list free of any derived KPI display", async () => {
    ...
    expect(
      screen.queryByText(
        /Tagesstand|Checkliste Heute|Überfällig|Diese Woche|Im Plan/,
      ),
    ).not.toBeInTheDocument();
  });
  ...
  it("source-locks the truthful station states without any KPI-derived display", () => {
    ...
    expect(source).toContain(
      "const [stationUnavailableMessage, setStationUnavailableMessage]",
    );
    expect(source).toContain(
