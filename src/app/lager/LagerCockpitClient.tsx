...
import { AlertTriangle, Box, Truck, FlaskConical, ArrowRight, Info, RefreshCw } from 'lucide-react';
import { FeedbackFooter } from "@/components/feedback/FeedbackFooter";
import { DetailOverlay } from '@/components/ui/DetailOverlay';
import type { InventoryItem } from '@/lib/repositories/inventoryRepository';
import { summarizeInventory } from '@/lib/inventory/inventorySummary';
import { subscribeInventorySync } from '@/lib/inventory/inventorySync';

interface Props {
  lagerData: InventoryItem[];
  loadError: string | null;
}

export function LagerCockpitClient({ lagerData, loadError }: Props) {
  ...
  const {
    criticalItems,
    chemicalItems: chemieItems,
    packagingItems: verpackungItems,
    recentStockInItems: recentItems,
    itemsWithoutMinStock,
  } = summarizeInventory(lagerData);

  useEffect(() => {
    const refresh = () => router.refresh();
    const unsubscribe = subscribeInventorySync(refresh);
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 md:p-6 lg:p-8 font-sans antialiased text-navy-900 min-h-screen bg-[#F0EBE0]">
      ...
      {loadError ? (
        <div role="alert" className="mb-12 rounded-2xl border border-error-red/30 bg-error-red/10 p-6 text-error-red">
          <h2 className="font-bold">Lagerdaten nicht verfügbar</h2>
          <p className="mt-1 text-sm">{loadError}</p>
          <p className="mt-2 text-xs">Bestände und Warnungen werden nicht als Nullwerte dargestellt.</p>
          <button type="button" onClick={() => router.refresh()} className="mt-4 inline-flex items-center gap-2 rounded-lg bg-error-red px-4 py-2 text-sm font-bold text-white">
      ...
      <>
      {lagerData.length === 0 && (
        <div className="mb-8 rounded-2xl border border-neutral-gray-200 bg-white p-5 text-sm text-text-muted">
          Der Lagerbestand wurde erfolgreich geladen; es sind noch keine Artikel erfasst.
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 mb-12">
        ...
        <button onClick={() => setActiveOverlay("critical")} className="text-left bg-white rounded-2xl p-5 border border-error-red/20 shadow-sm flex flex-col justify-between group hover:shadow-md transition-all cursor-pointer">
          <div>
            ...
            <p className="text-sm text-error-red font-medium">{criticalItems.length > 0
              ? "Nachbestellung nötig"
              : lagerData.length === 0
                ? "Keine Artikel erfasst"
                : itemsWithoutMinStock.length > 0
                  ? `${itemsWithoutMinStock.length} ohne Mindestbestand`
                  : "Kein Bestand unter Minimum"}</p>
          </div>
        ...
        <button onClick={() => setActiveOverlay("chemie")} className="text-left bg-white rounded-2xl p-5 border border-neutral-gray-200 shadow-sm flex flex-col justify-between group hover:shadow-md transition-all cursor-pointer">
          <div>
            ...
            <p className="text-sm text-text-muted font-medium">Erfasste Artikel</p>
          </div>
          <div className="mt-6 flex items-center justify-between w-full text-sm font-bold text-text-muted group-hover:text-navy-900 transition-colors">
            Details ansehen <ArrowRight className="w-4 h-4" />
          </div>
        ...
        <button onClick={() => setActiveOverlay("recent")} className="text-left bg-white rounded-2xl p-5 border border-success-green/20 shadow-sm flex flex-col justify-between group hover:shadow-md transition-all cursor-pointer">
          <div>
            ...
            <h3 className="font-bold text-navy-900 text-lg mb-1">Artikel mit Zugang</h3>
            <p className="text-sm text-success-green font-medium">In den letzten 5 Tagen</p>
          </div>
          <div className="mt-6 flex items-center justify-between w-full text-sm font-bold text-text-muted group-hover:text-navy-900 transition-colors">
            Details ansehen <ArrowRight className="w-4 h-4" />
          </div>
        ...
        <div className="space-y-6 text-navy-900">
          <div className="bg-error-red/10 border border-error-red/20 rounded-xl p-4 flex gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-error-red/80">{criticalItems.length === 0
                ? lagerData.length === 0
                  ? "Keine Lagerartikel erfasst."
                  : itemsWithoutMinStock.length > 0
                    ? `Kein bestätigter Mindestbestand ist unterschritten; für ${itemsWithoutMinStock.length} Artikel fehlt der Mindestbestand.`
                    : "Kein erfasster Artikel unterschreitet seinen Mindestbestand."
                : "Diese Artikel unterschreiten den konfigurierten Mindestbestand."}</p>
            </div>
          </div>
              ...
              {criticalItems.map((item) => (
                <li key={item.id} className="bg-white p-3 rounded-lg flex justify-between items-center border border-error-red/20">
                  <div>
                    ...
                    <p className="text-xs text-text-muted">Art-Nr: {item.sku || "nicht erfasst"}</p>
                  </div>
                  <div className="text-right">
                    <p className="font-bold text-error-red">{item.currentStock} {item.unit || "Einheit nicht erfasst"}</p>
                    <p className="text-xs text-text-muted">Min: {item.minStock ?? "nicht erfasst"} {item.unit || ""}</p>
                  </div>
                </li>
              ))}
            </ul>
        ...
        <div className="space-y-6 text-navy-900">
          <div className="bg-neutral-gray-100 border border-neutral-gray-200 rounded-xl p-4 flex gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-text-muted">{chemieItems.length === 0 ? "Keine Chemie-Artikel in der Datenbank." : "Alle erfassten Chemie-Artikel."}</p>
            </div>
          </div>
            ...
            <ul className="space-y-3">
              {chemieItems.map((item) => (
                <li key={item.id} className="bg-white p-3 rounded-lg flex justify-between items-center border border-neutral-gray-200">
                  ...
                  <div className="text-right">
                    <p className="font-bold">{item.currentStock} {item.unit || "Einheit nicht erfasst"}</p>
                  </div>
                </li>
              ))}
            </ul>
        ...
        <div className="space-y-6 text-navy-900">
          <div className="bg-neutral-gray-100 border border-neutral-gray-200 rounded-xl p-4 flex gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-text-muted">{verpackungItems.length === 0 ? "Keine Verpackungsartikel in der Datenbank." : "Alle erfassten Verpackungsartikel."}</p>
            </div>
          </div>
      ...
      <DetailOverlay open={activeOverlay === "recent"} onClose={closeOverlay} title="Artikel mit Zugang" subtitle="Artikel, deren letzter bestätigter Zugang höchstens fünf Tage zurückliegt.">
        <div className="space-y-6 text-navy-900">
          <div className="bg-success-green/10 border border-success-green/20 rounded-xl p-4 flex gap-3">
            ...
            <div>
              <h4 className="font-bold text-success-green">Letzte 5 Tage: {recentItems.length} Artikel</h4>
              <p className="text-sm text-success-green/80">{recentItems.length === 0 ? "Kein Artikel mit bestätigtem Zugang in diesem Zeitraum." : "Je Artikel wird der letzte bestätigte Zugang gezeigt; dies ist keine Anzahl einzelner Buchungen."}</p>
            </div>
          </div>
            ...
            <ul className="space-y-3">
              {recentItems.map((item) => (
                <li key={item.id} className="bg-white p-3 rounded-lg flex justify-between items-center border border-success-green/20">
                  <div>
                    <p className="font-bold">{item.name}</p>
                    <p className="text-xs text-text-muted">Eingang am: {item.lastStockInAt ? new Date(item.lastStockInAt).toLocaleDateString("de-DE") : "nicht erfasst"}</p>
                  </div>
                  <div className="text-right">
                    <p className="font-bold text-success-green">{item.currentStock} {item.unit || "Einheit nicht erfasst"}</p>
                  </div>
                </li>
              ))}
            </ul>
