      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <Link href="/" className="p-2 bg-kreile-surface-warm hover:bg-kreile-border text-kreile-muted rounded-lg mr-1 transition-colors">
              <ArrowLeft className="h-4 w-4" />
            </Link>
            <h1 className="text-2xl font-black text-kreile-navy">
              {getStationHeadline()}
            </h1>
          </div>
          <p className="text-kreile-muted text-sm mt-1">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        <div className="lg:col-span-2 space-y-3">
          ...
          {filteredOrders.length > 0 ? (
            <div className="space-y-6">
              {slug === "wareneingang" && (
                <Card className="bg-gradient-to-r from-kreile-navy to-kreile-navy-soft text-white p-6 rounded-xl shadow-md border border-white/10">
                  <div className="flex flex-col items-center">
                    ...
                    <Link href="/orders/new" className="w-full max-w-sm">
                      <Button className="w-full bg-white text-kreile-navy hover:bg-kreile-surface-warm py-6 text-base font-extrabold rounded-xl shadow-md transition-all">
                ...
                return (
                  <Card
                    ...
                    className={`transition-all duration-200 cursor-pointer shadow-sm ${itemBg} ${borderStyle} ${
                      isSelected ? "ring-2 ring-kreile-navy border-transparent shadow" : "border-kreile-border-strong"
                    }`}
                  >
                    <CardContent className="p-4 flex items-center justify-between gap-4">
                      ...
                      <div className="flex items-center gap-4 text-right shrink-0">
                        <div className="flex flex-col items-end">
                          <span className="text-[10px] text-kreile-muted font-bold uppercase">{dueLabel}</span>
                          <span className={`font-black text-lg ${isRed ? "text-red-600" : isOrange ? "text-orange-600" : "text-kreile-navy"}`}>
                            {dueValue}
                          </span>
                        </div>
        ...
        <div className="lg:col-span-1">
          {selectedOrder ? (
            <Card className="shadow-md border-kreile-border overflow-hidden sticky top-6">
              ...
              <CardContent className="p-5 space-y-5">
                <div className="grid grid-cols-2 gap-3 bg-kreile-surface-soft p-3 rounded-lg border border-slate-100 text-xs">
                  ...
                  <div>
                    <span className="text-kreile-muted block font-semibold text-[10px] uppercase">Teile</span>
                    <span className="font-bold text-kreile-navy">{selectedOrder.parts.length} Stück</span>
                  </div>
                </div>

                <div className="space-y-2 pt-2">
                  <span className="text-[10px] font-bold uppercase text-kreile-muted tracking-wider">Erfasste Werkstücke</span>
                  {(selectedOrder.parts || []).map((p, i) => (
                  <div key={p.id ?? `part-${i}`} className="p-2.5 bg-kreile-surface-soft border border-slate-100 rounded-lg flex items-center justify-between text-xs">
                    <div>
                      <span className="font-bold text-kreile-navy block">{p.name}</span>
                      <span className="text-kreile-muted text-[10px]">Material: {p.material} | Finish: {p.finish}</span>
                    </div>
                ...
                <div className="pt-4 border-t border-slate-100">
                  <Link href={`/orders/${selectedOrder.id}`}>
                     <Button className="w-full bg-kreile-navy hover:bg-kreile-navy-soft text-white shadow-sm font-bold h-11 rounded-xl">
            ...
            <Card className="border-dashed border-2 border-kreile-border-strong bg-kreile-surface-soft/50 p-6 text-center rounded-2xl space-y-2 sticky top-6">
              ...
              <h4 className="font-extrabold text-sm text-kreile-navy">Wähle einen Auftrag</h4>
