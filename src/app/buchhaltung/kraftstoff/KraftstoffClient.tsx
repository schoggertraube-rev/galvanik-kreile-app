...
export function KraftstoffClient({ initialTankungen, gesamtLiter, gesamtKosten }: { initialTankungen: BelegDetail[], gesamtLiter: number, gesamtKosten: number }) {
  usePageView();
  const [filter, setFilter] = useState("alle");

  // Map backend BelegDetail to the UI shape
  const tankungenUi = initialTankungen.map(t => ({
    id: t.id,
    kfz: t.kraftstoffDetail?.ort || "Unbekanntes KFZ", // We don't have KFZ field directly, use ort for now or similar
    date: new Date(t.belegdatum || t.erfasstAm).toLocaleDateString("de-DE"),
    liter: t.kraftstoffDetail?.liter || 0,
    amount: t.brutto || 0,
    sorte: t.kraftstoffDetail?.sorte || "Diesel",
    ort: t.lieferantText || t.kraftstoffDetail?.tankstelle || "Tankstelle",
    absetzbar: t.absetzbarProzent ? t.absetzbarProzent > 0 : true
  }));

  const filtered = filter === "alle" 
    ? tankungenUi 
    : tankungenUi.filter(t => t.kfz.includes(filter));
