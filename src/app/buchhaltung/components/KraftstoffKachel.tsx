...
import { getKraftstoffAnalysisAction } from "@/app/buchhaltung/analysis.actions";
...
export function KraftstoffKachel() {
  ...
  useEffect(() => {
    if (!open || data) return;
    const now = new Date();
    getKraftstoffAnalysisAction(`${now.getFullYear()}-01-01`, `${now.getFullYear()}-12-31`).then(setData);
  }, [open, data]);
  ...
  const props = !data ? { subtitle: "Daten werden live berechnet..." } : {
    ...
    linkedAreas: [{ label: "Kraftstoff Detailansicht", href: "/buchhaltung/kraftstoff" }]
  };
