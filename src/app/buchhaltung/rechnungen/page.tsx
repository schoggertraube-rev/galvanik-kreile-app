import { listOffenePostenAction, listRechnungenAction } from "@/app/buchhaltung/actions";
...
export default async function RechnungenPage({ searchParams }: { searchParams: Promise<{ [key: string]: string | string[] | undefined }> }) {
  ...
  const requestedStatus = typeof sp.status === "string" && INVOICE_STATUSES.has(sp.status as AusgangsrechnungStatus)
    ...
    : undefined;
  const openItemsView = sp.view === "open_items" || sp.filter === "offen";
  ...
  const initialRechnungen = openItemsView && !requestedStatus
    ? await listOffenePostenAction()
    : await listRechnungenAction(filter);
  ...
  return <RechnungenClient initialRechnungen={initialRechnungen} offeneSumme={offeneSumme} ueberfaelligSumme={ueberfaelligSumme} />;
}
