...
import { orders, customers, baeder, inventoryItems, phoneNotes } from "@/db/schema";
...
export async function globalSearchAction(term: string) {
  ...
  try {
    const [belege, foundOrders, foundCustomers, rechnungen, lieferanten, foundBaths, lager, kosten, notizen, zahlungen] = await Promise.all([
      ...
      db.select({ id: baeder.id, title: baeder.name, subtitle: baeder.status }).from(baeder).where(ilike(baeder.name, safeTerm)).limit(3).then(res => res.map(r => ({ ...r, type: 'bad' }))),
    ...
    return [...belege, ...foundOrders, ...foundCustomers, ...rechnungen, ...lieferanten, ...foundBaths, ...lager, ...kosten, ...notizen, ...zahlungen];
  } catch (err) {
