...
export function PriceAgreementPanel({ agreements }: { agreements: PriceAgreement[] }) {
  ...
  return (
    <div className="bg-amber-50 border-2 border-amber-200 rounded-3xl p-6 md:p-8 shadow-sm">
      <h3 className="text-sm font-extrabold text-amber-600 uppercase tracking-widest pl-1 mb-4 flex items-center">
        <FileBadge className="w-5 h-5 mr-2" /> Aktive Preisabsprachen
      </h3>
      <div className="space-y-4">
        {agreements.map(a => (
          <div key={a.id} className="bg-white p-5 rounded-2xl border-2 border-amber-100 shadow-sm">
            <div className="flex justify-between items-start">
              <div>
                <h4 className="font-bold text-slate-800 text-lg">{a.title}</h4>
                <p className="text-sm text-slate-500 font-bold mt-0.5">{a.surfaceType}</p>
              </div>
