  ...
  return (
    <div className="bg-white border-2 border-slate-200 rounded-3xl p-6 md:p-8 shadow-sm space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="flex items-center text-kreile-muted bg-slate-50 p-3 rounded-xl border border-slate-100">
          <Mail className="w-5 h-5 mr-3 text-kreile-muted" />
          <span className="font-bold text-sm truncate">{customer.email || "Keine E-Mail"}</span>
        </div>
        <div className="flex items-center text-kreile-muted bg-slate-50 p-3 rounded-xl border border-slate-100">
          <Phone className="w-5 h-5 mr-3 text-kreile-muted" />
          <span className="font-bold text-sm truncate">{customer.phone || "Keine Nummer"}</span>
        </div>
        <div className="flex items-center text-kreile-muted bg-slate-50 p-3 rounded-xl border border-slate-100">
          <MapPin className="w-5 h-5 mr-3 text-kreile-muted" />
          <span className="font-bold text-sm truncate">{customer.address || "Keine Adresse"}</span>
        </div>
      </div>

      <div className="flex gap-4 pt-2">
        <Button className="h-14 px-6 rounded-2xl bg-kreile-navy text-white font-bold text-lg hover:bg-slate-800 shadow-xl active:scale-95 transition-all">
          Neuer Auftrag
        </Button>
        <Button variant="outline" className="h-14 px-6 rounded-2xl border-2 border-slate-200 text-kreile-navy font-bold text-lg hover:bg-slate-50 active:scale-95 transition-all">
          Anrufen
        </Button>
      </div>
    </div>
