...
export function LabelPrintView({ order, customerName: propCustomerName, onClose, showPreviewModal = true }: LabelPrintViewProps) {
  ...
  const labelContent = (
    <div className="print-area font-sans text-black hidden print:flex flex-col justify-between h-[148mm] w-[105mm] p-6 bg-white border border-neutral-gray-100">
      ...
      <div className="space-y-4">
        ...
        <div className="text-center py-4 border-b border-black border-dashed">
          <div className="text-[12px] font-bold text-text-muted uppercase tracking-widest">AUFTRAGSNUMMER</div>
          <div className="text-[44px] font-black tracking-tight leading-none my-1">{order.orderNumber}</div>
          <div className="text-[14px] font-bold mt-1 text-navy-900">{order.title}</div>
        </div>

        {/* Details Table */}
        <div className="grid grid-cols-2 gap-y-3 text-[12px] py-2 border-b border-black">
          <div>
            <div className="text-[10px] font-bold text-navy-500 uppercase">KUNDE</div>
            <div className="font-black text-[13px]">{customerName}</div>
          </div>
          <div>
            <div className="text-[10px] font-bold text-navy-500 uppercase">DATUM</div>
            <div className="font-bold">{dateStr}</div>
          </div>
          <div className="col-span-2">
            <div className="text-[10px] font-bold text-navy-500 uppercase">BAUTEILE</div>
            <div className="space-y-1 mt-1 font-bold">
              {order.parts?.map((p: Record<string, unknown>, i: number) => (
                <div key={i} className="flex justify-between text-[11px] border-b border-neutral-gray-100 last:border-0 py-0.5">
                  <span>{String(p.quantity)}x {String(p.name)}</span>
                  {!!p.surfaceRequested && <span className="font-mono text-[9px] bg-neutral-gray-100 px-1 rounded">{String(p.surfaceRequested)}</span>}
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="flex flex-col items-center gap-2 border-t border-black pt-4">
        ...
        <div className="text-[9px] text-navy-500 font-bold uppercase tracking-wider mt-1">
  ...
  return (
    <>
      ...
      {showPreviewModal && (
        <div className="fixed inset-0 bg-navy-900/80 backdrop-blur-md flex items-center justify-center p-4 z-50 animate-in fade-in duration-300">
          <div className="bg-navy-900 border-2 border-navy-900 rounded-3xl w-full max-w-lg p-6 shadow-2xl space-y-6 flex flex-col max-h-[90vh] overflow-y-auto">
            {/* Modal Header */}
            <div className="flex justify-between items-center border-b border-navy-900 pb-4">
              <div className="flex items-center gap-2 text-white">
                <Printer className="w-5 h-5 text-navy-700" />
                <h3 className="text-xl font-bold tracking-tight">Etikettendruck Vorschau</h3>
              </div>
              {onClose && (
                <button onClick={onClose} className="p-2 hover:bg-navy-900 rounded-xl text-text-muted hover:text-white transition-colors">
                  <X className="w-5 h-5" />
                </button>
              )}
            </div>
            ...
            <div className="flex justify-center py-4 bg-navy-900 rounded-2xl border-2 border-navy-900/50 shadow-inner">
              <div className="bg-white text-black p-6 w-[280px] h-[395px] flex flex-col justify-between shadow-xl rounded-xl border border-neutral-gray-100 scale-95 origin-center">
                <div className="space-y-3">
                  ...
                  <div className="text-center py-2 border-b border-black border-dashed">
                    ...
                    <div className="text-[10px] font-bold text-navy-900 line-clamp-1">{order.title}</div>
                  </div>

                  <div className="space-y-1.5 text-[9px]">
                    <div className="flex justify-between">
                      <span className="text-navy-500 font-bold">KUNDE:</span>
                      <span className="font-black text-navy-900">{customerName}</span>
                    </div>
                    <div className="flex justify-between">
            ...
            <div className="flex gap-4">
              {onClose && (
                <Button variant="outline" onClick={onClose} className="flex-1 h-14 rounded-2xl border-2 border-navy-900 text-text-muted hover:bg-navy-900 hover:text-white font-bold transition-all">
                  Schließen
                </Button>
              )}
              <Button 
...
const QRCodeImage = ({ orderId, orderNumber }: { orderId: string; orderNumber: string }) => {
  ...
  if (!dataUrl) {
    return <div className="w-16 h-16 bg-neutral-gray-100 flex items-center justify-center rounded"><Loader2 className="animate-spin text-text-muted w-4 h-4" /></div>;
  }
