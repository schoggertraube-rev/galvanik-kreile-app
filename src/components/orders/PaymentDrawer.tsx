...
type PaymentDrawerProps = {
  orderData: {
    id?: string;
    priceLines?: ReadonlyArray<{ unitTotalEur?: string | number | null }>;
  };
  onClose: () => void;
};

export function PaymentDrawer({ orderData, onClose }: PaymentDrawerProps) {
  ...
  const totalValue = (orderData.priceLines ?? []).reduce(
    (sum, line) => sum + Number(line.unitTotalEur ?? 0),
    0,
  );

  const handleGenerateLink = async () => {
    if (!orderData.id) {
      setError("Auftrag nicht verfügbar");
      return;
    }
    setLoading(true);
    setError(null);
    setQrDataUrl(null);
    try {
      const res = await paymentProvider.createPaymentIntent({ orderId: orderData.id });
      if (!res.success) {
        setError(res.error);
        return;
      }
      setCheckoutUrl(res.checkoutUrl);
      setConfirmedAmountCents(res.amountCents);
      try {
        setQrDataUrl(await QRCode.toDataURL(res.checkoutUrl, {
          width: 200,
          margin: 1,
          errorCorrectionLevel: 'M',
        }));
      } catch {
        setError("Zahlungslink erstellt; QR-Code konnte lokal nicht erzeugt werden.");
      }
    } finally {
      setLoading(false);
    }
      ...
      <div className="w-full max-w-[560px] mx-4 bg-[var(--ci-surface)] rounded-[18px] border border-[var(--ci-border)] shadow-[0_1px_2px_rgba(20,15,5,0.04),0_12px_32px_rgba(20,15,5,0.08)]" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          <div className="bg-[var(--ci-surface)] border border-[var(--ci-border)] rounded-[14px] p-5">
            <div className="text-[var(--ci-ink-3)] text-xs uppercase tracking-wider mb-1">
              {confirmedAmountCents === null ? "Betrag aus aktueller Ansicht" : "Serverseitig bestätigter Zahlbetrag"}
            </div>
            <div className="font-serif text-3xl text-[var(--ci-ink)]">
              {confirmedAmountCents === null
                ? `${totalValue.toFixed(2)} €`
                : `${(confirmedAmountCents / 100).toFixed(2)} €`}
            </div>
          </div>
          ...
          {!checkoutUrl && (
            <div className="space-y-3">
              <button 
                ...
                disabled={loading || totalValue === 0 || !orderData.id}
              ...
              <button 
                ...
                title="Kein Tap-to-Pay-Connector konfiguriert"
              >
                <CreditCard className="w-5 h-5"/> Tap-to-Pay (NFC) nicht angebunden
              </button>
            </div>
          )}
            ...
            <div className="space-y-4 animate-in fade-in zoom-in duration-300">
              {qrDataUrl && (
                <div className="p-4 bg-white rounded-xl border border-[var(--ci-border)] flex items-center justify-center">
                  {/* eslint-disable-next-line @next/next/no-img-element -- local data URL generated in-browser */}
                  <img src={qrDataUrl} alt="Mollie-Zahlungslink als QR-Code" className="w-48 h-48" />
                </div>
              )}
