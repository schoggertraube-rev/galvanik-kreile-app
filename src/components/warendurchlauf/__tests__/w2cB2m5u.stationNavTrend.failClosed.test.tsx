...
vi.mock("next/navigation", () => ({
  usePathname: () => "/warendurchlauf/galvanik",
}));
vi.mock("next/link", () => ({
  default: ({ children }: { children: React.ReactNode }) => <a>{children}</a>,
}));
vi.mock("next/image", () => ({ default: () => null }));
vi.mock("@/lib/warendurchlaufIconResolver", () => ({
  getCurrentTimeOfDay: () => "noon",
  getCurrentWeather: () => "normal",
  getWareneingangVolumeState: () => "normal",
  getStationIcon: () => "/x.png",
}));
...
const input = (intakeDate: unknown) => ({
  id: "o1",
...
describe("W2C-B2M5U station trend truth", () => {
  it("renders loading without empty, chips, or trend truth", () => {
    ...
    expect(
      screen.getByText("Stationsdaten werden geladen..."),
    ).toBeInTheDocument();
  ...
  it("renders rejection fail-closed without stale zero chips or trends", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByText(
          "NOT_AVAILABLE: Stationsdaten konnten nicht geladen werden.",
        ),
      ).toBeInTheDocument(),
    );
  ...
  it("renders the genuine empty state after successful empty data", async () => {
    ...
    render(<WarendurchlaufStationNav />);
    expect((await screen.findAllByText("Keine Aufträge")).length).toBe(2);
    expect(screen.getByText("Wareneingang")).toBeInTheDocument();
    expect(screen.getByText("Galvanik")).toBeInTheDocument();
    expect(screen.queryByText("Warenausgang")).not.toBeInTheDocument();
  });

  it.each([undefined, "", "   ", "invalid"])(
    "shows unrecorded intake %p without bars or a current-date grouping",
    async (intakeDate) => {
      ...
      expect(
        container.querySelectorAll(
          'div[class~="w-1.5"][class~="rounded-t-sm"]',
        ),
      ).toHaveLength(0);
    },
  );
  ...
  it("renders seven real trend bars and the exact persisted-date total", async () => {
    ...
    expect(
      container.querySelectorAll('div[class~="w-1.5"][class~="rounded-t-sm"]'),
    ).toHaveLength(7);
  });
});
