    ...
    <RealtimeSyncProvider>
      ...
      >
        ...
        <div className="flex flex-1 min-h-0">   {/* min-h-0 verhindert Flex-Overflow */}
          ...
          <main
            className={`flex-1 overflow-y-auto overflow-x-hidden ${pathname.startsWith('/warendurchlauf') ? "" : "p-4 md:p-6 lg:p-8"}`}
          >
            {/* Max-Width Container — auf großen Screens zentriert */}
            <div className={`${pathname.startsWith('/warendurchlauf') ? "w-full h-full" : "max-w-[1400px] mx-auto w-full"} pb-24 md:pb-0`}>
              {children}
            </div>
          </main>
        </div>
