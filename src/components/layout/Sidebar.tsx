  ...
  return (
    <aside className="w-64 border-r border-slate-200 bg-white hidden md:flex flex-col h-full shrink-0">
      <div className="flex-1 py-6 flex flex-col gap-1 px-3 overflow-y-auto">
        <div className="px-3 mb-2 text-xs font-semibold text-kreile-muted uppercase tracking-wider">
          Übersicht
        </div>
          ...
          return (
            <Link
              ...
              className={`flex items-center justify-between px-3 py-2.5 rounded-md text-sm font-medium transition-colors ${
                isActive 
                  ? 'bg-slate-100 text-kreile-navy font-semibold' 
                  : 'text-kreile-muted hover:bg-slate-50 hover:text-slate-900'
              }`}
            >
              <div className="flex items-center gap-3">
                <item.icon className={`w-5 h-5 ${isActive ? 'text-blue-600' : 'text-kreile-muted'}`} />
                {item.name}
              </div>
      ...
      <div className="p-4 border-t border-slate-200">
        <Link
          href="/settings"
          className="flex items-center gap-3 px-3 py-2.5 rounded-md text-sm font-medium text-kreile-muted hover:bg-slate-50 hover:text-slate-900 transition-colors"
        >
          <Settings className="w-5 h-5 text-kreile-muted" />
          Einstellungen
        </Link>
      </div>
    </aside>
