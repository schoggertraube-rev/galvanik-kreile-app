  ...
  return (
    <div className="w-full max-w-3xl mx-auto space-y-6 animate-in slide-in-from-right-8 duration-300">
      <div className="text-center space-y-1 mb-6">
        <h2 className="text-3xl font-black font-serif text-slate-900">Scan-Ergebnis prüfen</h2>
        <p className="text-slate-500 font-medium">Bitte kontrolliere die von der KI erfassten Daten.</p>
      </div>

      {hasUncertain && (
        <div className="bg-orange-50 border-2 border-orange-300 rounded-2xl p-5 flex gap-4 items-center shadow-sm">
          <div className="bg-orange-100 p-3 rounded-full">
            <AlertTriangle className="text-orange-600 h-6 w-6" />
          </div>
          <div>
            <h4 className="font-extrabold text-orange-950">Prüfung erforderlich</h4>
            <p className="text-sm font-medium text-orange-800 mt-1">Die gelb markierten Felder sind unsicher. Bitte manuell korrigieren.</p>
          </div>
        </div>
      )}
      ...
      <div className="space-y-4">
        {fields.map((f, i) => {
          ...
          return (
            <div 
              key={f.key} 
              className={`p-4 rounded-xl border-2 transition-all ${isUncertain ? 'bg-yellow-50 border-yellow-400 shadow-md scale-[1.01]' : 'bg-white border-slate-200 focus-within:border-blue-500'}`}
            >
              ...
              <input 
                ...
                className={`w-full text-lg font-bold bg-transparent outline-none ${isUncertain ? 'text-yellow-900' : 'text-slate-900'}`}
              />
            </div>
          );
        })}
      ...
      <div className="pt-6">
        <Button 
          ...
          className={`w-full h-16 text-lg font-extrabold rounded-2xl shadow-lg transition-all ${
            hasUncertain ? "bg-slate-300 text-slate-500 hover:bg-slate-400" : "bg-green-600 text-white hover:bg-green-700"
          }`}
        >
