  ...
  return (
    <div className="w-full max-w-3xl mx-auto space-y-6 animate-in slide-in-from-right-8 duration-300">
      <div className="text-center space-y-1 mb-6">
        <h2 className="text-3xl font-black font-serif text-navy-900">Kunde zuordnen</h2>
        <p className="text-navy-500 font-medium">Wir haben nach &quot;{ocrData.customerName}&quot; in der Kartei gesucht.</p>
      </div>
        ...
        <div className="space-y-4">
          {matches.length > 0 ? (
            <div className="space-y-3">
              <h3 className="font-bold text-navy-900 text-sm uppercase tracking-wider pl-1">Gefundene Treffer</h3>
              {matches.map(m => (
                <button 
                  key={m.id}
                  onClick={() => onConfirm(m.id)}
                  className="w-full text-left bg-white border-2 border-neutral-gray-100 hover:border-navy-700 hover:bg-gold-100 hover:shadow-md p-4 rounded-2xl flex items-center justify-between transition-all active:scale-98"
                >
                  <div>
                  ...
                  <UserCheck className="h-6 w-6 text-navy-700" />
                </button>
              ))}
            </div>
          ) : (
            <div className="bg-bg-app-soft border-2 border-neutral-gray-100 border-dashed rounded-3xl p-8 text-center">
              ...
              <p className="text-navy-500 mt-1 font-medium">Soll &quot;{ocrData.customerName}&quot; neu angelegt werden?</p>
            </div>
          )}

          <div className="pt-6 border-t border-neutral-gray-100 mt-6">
            <Button 
              onClick={() => onConfirm(null, ocrData.customerName)}
              variant="outline"
              className="w-full h-16 text-lg font-extrabold rounded-2xl border-2 border-text-muted hover:bg-neutral-gray-100 text-navy-900 active:scale-95 transition-all"
            >
