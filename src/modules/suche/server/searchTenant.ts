...
function assertOrder(document: SearchOrderDocument): void {
  if (
    !document || !validRequired(document.id) || !validRequired(document.orderNumber)
    || !validOptional(document.customerName) || !validRequired(document.title)
    || !validOptional(document.task) || !validRequired(document.station)
    || !validRequired(document.status) || typeof document.dueDate !== "string"
...
function assertCustomer(document: SearchCustomerDocument): void {
  if (
    !document || !validRequired(document.id) || !validOptional(document.customerNumber)
    || !validRequired(document.name) || !validOptional(document.companyName)
    || !validRequired(document.customerType) || !validOptional(document.city)
...
function toOrderHit(order: SearchOrderDocument, matchField: SearchMatchField, query: string): SearchHit {
  ...
  return {
    type: "ORDER", id: order.id, title: order.title,
    subtitle: [order.orderNumber, order.customerName, dueDate ? `Termin ${dueDate}` : null, order.station].filter(Boolean).join(" · "),
...
function toCustomerHit(customer: SearchCustomerDocument, matchField: SearchMatchField): SearchHit {
  return {
    type: "CUSTOMER", id: customer.id, title: customer.companyName ?? customer.name,
    subtitle: [customer.customerNumber, customer.city].filter(Boolean).join(" · "), status: customer.customerType,
...
function stableHits(hits: SearchHit[]): SearchHit[] {
  return hits.sort((left, right) => left.title.localeCompare(right.title, "de-DE", { sensitivity: "base" }) || left.id.localeCompare(right.id));
}
