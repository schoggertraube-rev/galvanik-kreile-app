...
const PATH_PATTERN =
  /^order-station-evidence\/v1\/[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}\.(?:jpg|png|webp)$/;
const ACCEPTED_MIME = new Set<OrderStationAttachmentMime>([
  "image/jpeg",
  "image/png",
  "image/webp",
]);
...
export type OrderStationAttachmentPanelProps = {
  orderId: string;
  expectedVersion: number;
  items: AttachmentItem[];
  ports: OrderStationAttachmentPorts;
};

type MetadataResult = OrderStationAttachmentResult<{
  receipts: OrderStationAttachmentReceipt[];
  evidenceRecords: EvidenceReadRecord[];
  canOperate: boolean;
  currentActorId: string;
}>;

type ReserveResult = OrderStationAttachmentResult<{
  receipt: OrderStationAttachmentReceipt;
  upload: { path: string; token: string } | null;
  replayed: boolean;
}>;

type FinalizeResult = OrderStationAttachmentResult<{
  receipt: OrderStationAttachmentReceipt;
  replayed: boolean;
}>;

type OriginalResult = OrderStationAttachmentResult<{
  downloadUrl: string;
  expiresInSeconds: number;
  mimeType: OrderStationAttachmentMime;
}>;

export type OrderStationAttachmentPorts = {
  read(input: GetOrderStationAttachmentsInput): Promise<MetadataResult>;
  reserve(input: ReserveOrderStationAttachmentInput): Promise<ReserveResult>;
  finalize(input: FinalizeOrderStationAttachmentInput): Promise<FinalizeResult>;
  readOriginal(
  ...
  uploadSigned(input: {
    ...
    token: string;
    bytes: Uint8Array;
    contentType: OrderStationAttachmentMime;
  }): Promise<{ data: { path: string } | null; error: unknown | null }>;
};

async function readAndHash(
  file: File,
): Promise<{ bytes: Uint8Array; contentSha256: string }> {
...
function sameReceipt(
  ...
  return (
    sameImmutableBinding(left, right) &&
...
function submittedIntentMatchesReceipt(
  ...
  return (
    UUID_PATTERN.test(receipt.reservationId) &&
    receipt.actorId === actorId &&
    receipt.orderId === orderId &&
    receipt.itemId === itemId &&
    receipt.orderVersion === expectedVersion &&
...
function newestPending(
  receipts: OrderStationAttachmentReceipt[],
): OrderStationAttachmentReceipt | null {
  return (
    [...receipts].sort((left, right) => {
      const byTime =
        new Date(right.reservedAt).getTime() -
        new Date(left.reservedAt).getTime();
      return byTime !== 0
        ? byTime
        : right.reservationId.localeCompare(left.reservationId);
    })[0] ?? null
  );
}
...
function evidenceRecordsMatchScope(
  ...
  return records.every(
    (record) =>
      record.targets.some(
        (target) =>
          target.targetType === "ORDER" && target.targetId === orderId,
      ) &&
      (record.source !== "ORDER_STATION_ATTACHMENT" ||
        record.targets.some(
          (target) =>
            target.targetType === "ORDER_ITEM" && target.targetId === itemId,
        )),
  );
}
...
export function OrderStationAttachmentPanel({
  orderId,
  expectedVersion,
  items,
  ports,
  ...
  const panelKey = JSON.stringify([
    orderId,
    expectedVersion,
    attachmentItemsIdentity(items),
  ]);
  return (
    <OrderStationAttachmentPanelInner
      key={panelKey}
      orderId={orderId}
...
function OrderStationAttachmentPanelInner({
  orderId,
  expectedVersion,
  items,
  ports,
  ...
  const [restartSelection, setRestartSelection] =
    useState<RestartSelection | null>(null);
  ...
  const renderedScopeKey = makeScopeKey(
    orderId,
    expectedVersion,
    selectedItemId,
    itemIdentity,
  ...
  const scopeKeyRef = useRef(
    makeScopeKey(orderId, expectedVersion, initialItemId, itemIdentity),
  );
  ...
  const baseScopeIsCurrent = useCallback(
    (scope: Pick<WorkflowScope, "key" | "generation">): boolean =>
      scope.key === scopeKeyRef.current &&
      scope.generation === scopeGeneration.current,
    [],
  );

  const workflowIsCurrent = useCallback(
    (scope: WorkflowScope): boolean =>
      baseScopeIsCurrent(scope) &&
      activeOperationId.current === scope.operationId,
    [baseScopeIsCurrent],
  );
  ...
  const invalidateScope = (nextItemId: string) => {
    ...
    scopeKeyRef.current = makeScopeKey(
      orderId,
      expectedVersion,
      nextItemId,
      itemIdentity,
  ...
  const loadReceipts = useCallback(
    async (
      ...
      setServerCanOperate(false);
      try {
        const result = await ports.read({ orderId, itemId: selectedItemId });
        if (
        ...
        if (
          result.data.receipts.some(
            (receipt) =>
              receipt.orderId !== orderId || receipt.itemId !== selectedItemId,
          ) ||
          !evidenceRecordsMatchScope(
            result.data.evidenceRecords,
            orderId,
            selectedItemId,
          )
        ) {
          ...
          setMessage(
            preserveMessage ??
              "Metadaten waren nicht exakt an Auftrag und Teil gebunden.",
          );
          setPanelState("unavailable");
          setLoadedScopeKey(requestScope.key);
          return null;
        ...
        setCurrentActorId(null);
        setMessage(
          preserveMessage ?? "Übergabebelege sind derzeit nicht verfügbar.",
        );
        setPanelState("unavailable");
        setLoadedScopeKey(requestScope.key);
        return null;
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      ...
      setMessage((current) =>
        current === "Privater Download ist kurzzeitig verfügbar."
  ...
  const confirmFreshReceipt = async (
    ...
    const sequence = ++metadataSequence.current;
    const result = await ports.read({ orderId, itemId: selectedItemId });
    if (!workflowIsCurrent(scope) || sequence !== metadataSequence.current)
      return false;
    if (
      result.code !== "OK" ||
      ...
      return false;
    const fresh = result.data.receipts.find(
      (receipt) => receipt.reservationId === expected.reservationId,
    );
    if (
      !fresh ||
  ...
  const attemptFinalize = async (
    ...
    if (finalized.code !== "OK") {
      if (
        finalized.code === "CONFLICT" &&
        RESTART_REASONS.has(finalized.reason)
      ) {
      ...
      return finalized.code === "CONFLICT" &&
        finalized.reason === "UPLOAD_NOT_READY"
        ? "NOT_READY"
        : "STOP";
    }
    if (
      !finalizedReceiptMatchesSource(finalized.data.receipt, source, actorId)
    ) {
      await refreshAfterFailure(
        scope,
        "Bestätigung war nicht exakt an die Reservierung gebunden. Kein Erfolg wurde angezeigt.",
      );
      return "STOP";
    }
    if (!(await confirmFreshReceipt(finalized.data.receipt, scope))) {
      await refreshAfterFailure(
        scope,
        "Metadaten-Bestätigung ist fehlgeschlagen. Kein Erfolg wurde angezeigt.",
      );
      return workflowIsCurrent(scope) ? "STOP" : "STALE";
    }
    if (!workflowIsCurrent(scope)) return "STALE";
    setRestartSelection((current) =>
      current?.reservationId === source.reservationId ? null : current,
    );
    setMessage(
  ...
  const reserveUploadFinalize = async (
    ...
    const reserved = await ports.reserve({
      orderId,
    ...
    if (
      ...
      (replayReservationId !== undefined &&
        reservationReceipt.reservationId !== replayReservationId)
    ) {
      await refreshAfterFailure(
        scope,
        "Reservierungsantwort war nicht exakt gebunden. Kein Upload wurde gestartet.",
      );
      return;
    }
    ...
    if (reservationReceipt.state === "FINALIZED") {
      if (
        reserved.data.upload !== null ||
        !(await confirmFreshReceipt(reservationReceipt, scope))
      ) {
        await refreshAfterFailure(
          scope,
          "Finalisierter Beleg konnte nicht exakt bestätigt werden.",
        );
        return;
      }
      if (!workflowIsCurrent(scope)) return;
      setMessage(
        "Original war bereits sicher gespeichert und wurde erneut bestätigt.",
      );
      return;
    }

    const grant = reserved.data.upload;
    const exactPath = expectedObjectPath(
      reservationReceipt.reservationId,
      intent.mimeType,
    );
    if (
      !grant ||
      ...
      await refreshAfterFailure(
        scope,
        "Uploadfreigabe war nicht exakt gebunden. Kein Upload wurde gestartet.",
      );
      return;
    }

    setOperation("uploading");
    const uploaded = await ports.uploadSigned({
      bucketId: BUCKET_ID,
      path: grant.path,
      token: grant.token,
      bytes,
    ...
    if (uploaded.error || uploaded.data?.path !== grant.path) {
      await refreshAfterFailure(
        scope,
        "Upload konnte nicht sicher abgeschlossen werden.",
      );
      return;
    }
    ...
    if (
      !finalizedReceiptMatchesSource(
        finalized.data.receipt,
        reservationReceipt,
        actorId,
      )
    ) {
      await refreshAfterFailure(
        scope,
        "Bestätigungsantwort war nicht exakt gebunden. Kein Erfolg wurde angezeigt.",
      );
      return;
    }
    if (!(await confirmFreshReceipt(finalized.data.receipt, scope))) {
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      if (
        !ACCEPTED_MIME.has(file.type as OrderStationAttachmentMime) ||
        file.size < 1 ||
      ...
      await refreshAfterFailure(
        scope,
        "Uploadablauf ist derzeit nicht verfügbar. Kein Erfolg wurde angezeigt.",
      );
    } finally {
  ...
  const finalizeAndConfirm = async (receipt: OrderStationAttachmentReceipt) => {
    ...
    try {
      ...
      await refreshAfterFailure(
        scope,
        "Bestätigung ist derzeit nicht verfügbar. Kein Erfolg wurde angezeigt.",
      );
    } finally {
      finishWorkflow(scope);
    }
  ...
  const downloadOriginal = async (receipt: OrderStationAttachmentReceipt) => {
    ...
    try {
      ...
      const result = await ports.readOriginal({
      ...
      if (workflowIsCurrent(scope))
        setMessage("Originaldownload ist derzeit nicht verfügbar.");
    } finally {
      finishWorkflow(scope);
    }
  };
    ...
    >
      ...
      {items.length === 0 ? (
        ...
        >
          ...
          Kein eindeutig zugeordnetes Teil verfügbar. Upload bleibt gesperrt.
        </div>
      ) : items.length > 1 ? (
        ...
        <ul className="space-y-2">
          {receipts.map((receipt) => {
            const restartRequired =
              restartSelection?.reservationId === receipt.reservationId;
            ...
            return (
              ...
              >
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <span>
                    {receipt.mimeType} ·{" "}
                    {receipt.fileBytes.toLocaleString("de-DE")} Byte
                  </span>
                </div>
                <p className="mt-1 text-muted-foreground">
                  Reserviert: {formatTimestamp(receipt.reservedAt)} ·
                  Uploadfrist: {formatTimestamp(receipt.uploadExpiresAt)}
                </p>
                {restartRequired &&
                  restartSelection?.reason === "UPLOAD_GRANT_EXPIRED" && (
                    <p className="mt-1 text-destructive">
                      Bei erneuter Auswahl wird zuerst das alte Objekt geprüft;
                      fehlt es weiter, folgt bewusst eine neue Reservierung und
                      Uploadfreigabe.
                    </p>
                  )}
                {restartRequired &&
                  restartSelection?.reason !== "UPLOAD_GRANT_EXPIRED" && (
                    <p className="mt-1 text-destructive">
                      Immutable Reservierung nicht wiederverwendbar; keine
                      Adoption oder Überschreibung.
                    </p>
                  )}
                {receipt.state === "PENDING" && !ownedByCurrentActor && (
                  <p className="mt-1 text-muted-foreground">
                    Metadaten eines anderen Bearbeiters; keine Bedienrechte.
                  </p>
                )}
                {receipt.state === "PENDING" &&
                  ownedByCurrentActor &&
                  !currentPending && (
                    <p className="mt-1 text-muted-foreground">
                      Eigene historische Reservierung; aktueller Auftragsstand
                      weicht ab.
                    </p>
                  )}
                {scopeReady &&
      ...
      {selectedItem &&
        ...
        serverCanOperate && (
          <label
            className={`mt-3 flex items-center justify-center gap-2 rounded-md border border-dashed border-border p-2 text-xs ${canUpload ? "cursor-pointer" : "cursor-not-allowed opacity-60"}`}
          >
            {busy ? (
              <Loader2 className="h-4 w-4 animate-spin" />
            ) : (
      ...
      <div className="mt-2 min-h-4 text-xs" role="status" aria-live="polite">
        {scopeReady
          ? operationLabel(operation) ||
            (panelState !== "unavailable" ? message : "")
          : ""}
      </div>
    </section>
  );
