...
import { WareneingangHandoffButton } from "../legacy-ui/WareneingangHandoffButton";
  ...
  render(
    <WareneingangHandoffButton
      ...
      ports={{
        ...
        readSource: getWareneingangSpy,
        readTarget: getGalvanikSpy,
        readReceipt: getReceiptSpy,
      }}
  ...
  it("keeps the id for the same order/version retry and rotates it when the order changes", async () => {
    ...
    const { rerender } = render(
      <WareneingangHandoffButton
        orderId="order-1"
        expectedVersion={1}
        ports={{ transition: transitionSpy, readSource: getWareneingangSpy, readTarget: getGalvanikSpy, readReceipt: getReceiptSpy }}
        onConfirmedReadback={onConfirmedReadback}
      />,
    );
    const button = screen.getByRole("button", { name: "An Galvanik übergeben" });
