...
type Props = {
  ...
  ports: WareneingangHandoffPorts;
...
type HandoffCommandResult =
  | { code: "OK"; receipt: HandoffReceipt; replayed: boolean }
...
export type WareneingangHandoffPorts = {
  transition(input: { orderId: string; expectedVersion: number; clientEventId: string }): Promise<HandoffCommandResult>;
  readSource(): Promise<ReadResult<WarendurchlaufOrder[]>>;
  readTarget(): Promise<ReadResult<WarendurchlaufOrder[]>>;
  readReceipt(input: { orderId: string; clientEventId: string }): Promise<ReadResult<HandoffReceipt | null>>;
};

export type WareneingangHandoffButtonProps = Props;
...
export function WareneingangHandoffButton({
  ...
  async function handleHandoff() {
    ...
    try {
      ...
      const command = await ports.transition({
        orderId,
        expectedVersion,
        clientEventId,
      });
      ...
      if (command.code === "CONFLICT") {
        try {
          const sourceRead = await ports.readSource();
