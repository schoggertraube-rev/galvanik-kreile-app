...
import type { OperationalOrder as WarendurchlaufOrder } from "@/lib/types/operationalOrder";
import { ORDER_LIFECYCLE_STATUS } from "../domain/orderLifecycleContract";

type Props = {
  orderId: string;
  expectedVersion: number;
  ports: GalvanikCorrectionPorts;
...
type CorrectionResult =
  | { code: "OK"; receipt: CorrectionReceipt; replayed: boolean }
  | { code: "UNAUTHENTICATED" | "FORBIDDEN" | "NOT_FOUND" | "CONFLICT" | "VALIDATION_ERROR" | "UNAVAILABLE"; message: string };

type ReadResult<T> =
  | { ok: true; data: T }
  | { ok: false; error: string; message: string };

export type GalvanikCorrectionPorts = {
  correct(input: { orderId: string; expectedVersion: number; clientEventId: string; reason: string }): Promise<CorrectionResult>;
  readSource(): Promise<ReadResult<WarendurchlaufOrder[]>>;
  readTarget(): Promise<ReadResult<WarendurchlaufOrder[]>>;
  readReceipt(input: { orderId: string; clientEventId: string }): Promise<ReadResult<CorrectionReceipt | null>>;
};

export type GalvanikCorrectionButtonProps = Props;
...
export function GalvanikCorrectionButton({
  ...
  async function handleCorrect() {
    ...
    try {
      ...
      const command = await ports.correct({
        orderId,
        expectedVersion,
        clientEventId,
        reason: trimmedReason,
      ...
      if (command.code === "CONFLICT") {
        try {
          const targetRead = await ports.readTarget();
      ...
      let receiptRead;
      try {
        sourceRead = await ports.readSource();
        targetRead = await ports.readTarget();
        receiptRead = await ports.readReceipt({ orderId, clientEventId });
      } catch {
