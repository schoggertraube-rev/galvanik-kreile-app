...
export type GalvanikHandoffAttachmentPanelProps = {
  ...
  ports: GalvanikHandoffAttachmentPorts;
};
...
export type GalvanikHandoffAttachmentPorts = {
  ...
  readOriginal(input: GetOrderStationAttachmentOriginalInput): Promise<OriginalResult>;
  uploadSigned(input: {
    bucketId: string;
    path: string;
    token: string;
...
function GalvanikHandoffAttachmentPanelInner({
  ...
  const attemptFinalize = async (
    ...
    const finalized = await ports.finalize({ reservationId: source.reservationId });
  ...
  const downloadOriginal = async (receipt: OrderStationAttachmentReceipt) => {
    ...
    try {
      ...
      const result = await ports.readOriginal({ receiptId: receipt.receiptId! });
      if (!workflowIsCurrent(scope)) return;
      if (result.code !== "OK") {
