...
- **Spur B — Path-1 Modul-Umbau** (Architektur): **S0 + S1 in main integriert** (`160bcf4`, PR #75 gemerged). S2 (Kill-Liste) und S3 (Muster-Modul `erfassung`) sind **nicht begonnen** und werden **nicht vorgezogen**. Owner-priorisiert läuft stattdessen der **S4-Kandidat Phillip-Werkstatt-V4** auf Branch `path1/werkstatt-phillip-v4-20260907` (noch nicht reviewed/gemergt). Plan: `ARCHITEKTUR_MODULE_PATH1.md` §4.
...
- **Aktuell dran (Owner-priorisiert):** S4-Kandidat **Phillip-Werkstatt-V4** auf Branch `path1/werkstatt-phillip-v4-20260907` ist **gebaut**. Fokussierte Tests 29/29 grün, `module-gates` grün, NO_FAKE 0/0/PASS, `tsc` und `lint` lokal grün. Standard-Vitest/Build laufen lokal **nicht** — Sicherheitsprofil blockiert mit EPERM den Zugriff auf `.env.local` — und müssen daher im Exact-SHA-CI belegt werden. Nächster Gatepfad: echte Browser-Screenshots erstellen, dann genau **ein** Commit, Push, Draft-PR eröffnen, Exact-SHA-CI/Preview abwarten und unabhängig prüfen lassen. **Kein** S4-PASS/Merge behauptet.
