...
- **SUCHLEISTE** („das Gehirn", eigenes Modul, mit allem verknüpft; Pflicht vor Pilot F1.6): Suchvertrag — jedes Modul liefert Suchindex; Volltext über Kunde/Auftrag/Nummer/Termin. HINWEIS: Lieferung liegt jetzt IM Repo (`_lieferungen/suche/`, seit 2026-09-06) → nach `src/modules/suche/` forken (5 Nähte). Siehe `_lieferungen/suche/LIEFERSCHEIN.md`.
...
- **Integriert (S4, PR #77):** `src/modules/werkstatt/` traegt die Phillip-V4-UI; `/warendurchlauf` bleibt Kompositionswurzel und liest ueber die bestehenden `getWareneingangOrdersAction`/`getGalvanikOrdersAction`. D.1/D-UI-F15-004A ist mit PR #81 geliefert, bindet den `Ware raus`-Griff ueber einen engen Port an den gelieferten OrderOverlay-Fluss und filtert ausschliesslich die kanonische Station `fertig`. F1.5 A+B/B2+C+D+D.1+T ist nach PR #82 vollstaendig geliefert. F1.6 bleibt ungestartet und seine Pilot-Readiness `BLOCKED_PRODUCT_DECISION`, bis die hier als Pflicht vor Pilot benannten Module Suchleiste und Kalender real und modular geplant, gebaut und abgenommen sind.
