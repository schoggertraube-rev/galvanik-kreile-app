...
Gebaut+geprüft: Fundament (F0/F1.1), Domänen-Logik F1.4/F1.5 (#73 gemerged). **S0 (Tenant zentral) + S1 (Naht-Gates als CI: `npm run quality:module-gates`, Beweis-Tests `src/test/s1_module_gates.test.ts`) = gebaut, PR #75**, Red-Team bestanden, Merge = Owner. NICHT gebaut: `src/modules/` (S3 Muster-Modul), Kill-Liste (S2 — ihre 12 Dateien sind die Altlast-Baseline von Naht 5), die Home-Neubauten (S4), buchhaltung-Trim, Skonto. Suchleiste: Lieferung als Referenztext in `_lieferungen/suche/`, Fork nach `src/modules/suche/` in S5. Aktueller Stand + Leitplanken: `00_JETZT_UND_LEITPLANKEN.md`. Unabhängiger Rotstand (Prüfer ≠ Autor): `00_UNABHAENGIGE_PRUEFUNG_2026-09-06.md`.
