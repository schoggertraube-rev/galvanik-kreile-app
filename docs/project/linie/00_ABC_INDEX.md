...
Gebaut+geprüft: Fundament (F0/F1.1), F1.4 sowie F1.5 A+B/B2+C+D; C ist mit PR #79 (`b94821ca…`), D mit PR #80 (`001a7698…`) geliefert. **S0 + S1 + S4 sind integriert:** PR #75 lieferte Tenant-/Naht-Gates, PR #77 das erste Path-1-Modul `src/modules/werkstatt/` mit Phillip V4 und `/warendurchlauf` als Kompositionswurzel. Aktueller Kandidat ist D.1/D-UI-F15-004A: der `Ware raus`-Griff nutzt einen engen Port und bietet nur echte `fertig`-Auftraege an. T (Supabase-CLI-Pin) bleibt danach offen; noch kein D.1-/F1.5-Gesamt-PASS vor Merge. NICHT vorziehen: S2/S3, restliche Modul-Umzuege oder F1.6. Suchleiste: Lieferung als Referenztext in `_lieferungen/suche/`, Fork nach `src/modules/suche/` erst im freigegebenen Paket. Aktueller Stand + Leitplanken: `00_JETZT_UND_LEITPLANKEN.md`.
