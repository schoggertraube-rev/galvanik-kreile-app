...
## Status-Kurz (Stand 2026-09-07)
Gebaut+geprüft: Fundament (F0/F1.1), F1.4 sowie F1.5 A+B/B2 (#73 gemerged). F1.5 C Warenausgang/Gate und D UI sind offen. **S0 (Tenant zentral) + S1 (Naht-Gates als CI: `npm run quality:module-gates`, Beweis-Tests `src/test/s1_module_gates.test.ts`) = in main integriert (PR #75 gemerged, `160bcf4`)**. **Kandidat auf Branch `path1/werkstatt-phillip-v4-20260907` (noch nicht reviewed/gemergt): S4 Werkstatt-Modul** — `src/modules/werkstatt/` (Phillip V4), `/warendurchlauf` bleibt Kompositionswurzel, `WarendurchlaufCockpitClient.tsx`/`PhillipWerkstatt.module.css`/`ThemeToggle.tsx` gelöscht. NICHT gebaut: Kill-Liste (S2 — ihre verbleibenden Dateien sind die Altlast-Baseline von Naht 5), restliche Home-/Modul-Umzüge, buchhaltung-Trim, Skonto. Suchleiste: Lieferung als Referenztext in `_lieferungen/suche/`, Fork nach `src/modules/suche/` in S5. Aktueller Stand + Leitplanken: `00_JETZT_UND_LEITPLANKEN.md`. Unabhängiger Rotstand (Prüfer ≠ Autor): `00_UNABHAENGIGE_PRUEFUNG_2026-09-06.md`.
