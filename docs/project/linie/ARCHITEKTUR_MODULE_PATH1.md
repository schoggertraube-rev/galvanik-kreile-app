...
**Bewusste Grenzen des Gates (ehrlich, nach unabhängigem Red-Team 2026-09-06):** Naht 2 löst nur `@/` und relative Pfade auf — deshalb ist `tsconfig.paths` per Gate auf genau `"@/*": ["./src/*"]` festgenagelt; Symlinks müssen im Repo bleiben (sonst FAIL); `public.ts` darf kein `export *` enthalten (Fassade = explizite Liste). Naht 4 sieht nur schema-qualifiziertes Roh-SQL (`public.x`, `"public"."x"`, auch in `.sql`-Dateien) und Supabase `.from('x')`; unqualifizierte Tabellennamen, Drizzle-Tabellenobjekte und interpolierte Namen sieht es NICHT — dafür gilt PL-Review. Naht 5 fängt wörtliche Wiederverwendung (auch in Kommentaren/Strings → dann Kommentar ändern, nicht das Gate); `git mv` einer Altlast-Datei zählt als neue Datei und ist rot — Altlasten werden gelöscht, nicht verschoben.
...
- Die UI-Konvergenz ist ein kanonischer Vollersatz, keine Reparatur oder evolutionäre Umgestaltung der bestehenden sichtbaren Oberfläche. Alte Shell, Navigation, Startseite, Orders-/Customers-UI und responsive Navigation sind keine Architekturvorlage. Wiederverwendet werden nur belastbare Backend-, Daten- und Auth-Verträge sowie technisch sinnvolle Logik hinter der Oberfläche.
