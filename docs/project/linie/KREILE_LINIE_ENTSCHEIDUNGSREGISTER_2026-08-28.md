...
**E — Lieferreihenfolge und Gate:** Nach dem Governance-Paket steuert die Mission exakt `PATH1_UI_CONVERGENCE_A`, danach B Orders V8, C Customers V2, D Suche auf der Ziel-Shell erneut integrieren und abnehmen, E Kalender. A wird mit realen Logins aller sechs Rollen sowie vollständigen authentifizierten Routen bei `1914x917`, `1220x880` und `390x844` side-by-side abgenommen. Alle sichtbaren Navigationsziele werden geklickt, entfernte und quarantinisierte Direkt-URLs negativ geprüft, Alt-Shell-/Navigationsausnahmen shrink-only reduziert und die nummerierte Abweichungsliste enthält keinen P0/P1. PR #84 bleibt bis nach A bis C `DRAFT/CANDIDATE` und wird nicht in die Alt-Shell gemergt. F1.6 und Kalender bleiben nicht gestartet.
