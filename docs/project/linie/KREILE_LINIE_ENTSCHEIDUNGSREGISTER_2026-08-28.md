...
## D-UI-CORE-001 — Sichtbare Produktwahrheit und Path-1-UI-Konvergenz (Owner 2026-09-10)

**Entscheidung/Wortlaut:** Gelieferte Fachverträge und ein geliefertes Werkstatt-Innenmodul sind kein Beleg für eine gelieferte Gesamtoberfläche. Der belegte sichtbare Stand wird getrennt geführt: Backend/Fachverträge sind teilweise real; das S4-Werkstatt-Innenmodul ist geliefert; die gesamte Zieloberfläche ist `OWNER_UX_FAIL / NOT_DELIVERED`; Orders/Auftragskarte V8, Customers/Kundenkarte V2 sowie Rollen-Startseiten Phillip V4/Rolf V8 einschließlich Zielnavigation sind `NOT_DELIVERED`. Production zeigt weiterhin die verworfene Alt-Shell und darf weder als sichtbarer Zielstand noch als livefähige Gesamt-App berichtet werden. PR #84 bleibt `DRAFT/CANDIDATE` und wird nicht in die Alt-Shell gemergt. Sein Suchkern ist verwertbar; Header-/Overlay-Integration und vollständige Abnahme erfolgen erst auf der neuen kanonischen Shell.

**Verbindliche nächste Produktpriorität nach Abschluss des Governance-Pakets:** `PATH1_UI_CONVERGENCE` vor Kalender und F1.6, seriell in dieser Reihenfolge: A) globale Shell und Rollen-Startseiten nach Phillip V4/Rolf V8; alte Sidebar und Warendurchlauf-Routennavigation abbauen, entfallene oder quarantinisierte Module nicht als verfügbare Navigation zeigen. B) Orders auf die Auftragskarte V8 als verbindliche Hauptansicht konvergieren. C) Customers auf die Kundenkarte V2 als verbindliche Hauptansicht konvergieren. D) Suche erst danach in die Ziel-Kopfleiste integrieren und auf neuer Basis vollständig neu abnehmen. E) erst danach Kalender und weitere Module. HTML-Referenzen liefern keine Demo-Inhalte; sichtbar sind ausschließlich reale Daten oder ehrliche Leerzustände mit sinnvollen Primäraktionen.

**Verbindliche UI-Abnahme je Teilpaket:** Die vollständige authentifizierte Route umfasst Shell, Header, Navigation, Inhalt und Aktionsleiste. Sie wird bei Desktop `1914x917`, Tablet `1220x880` und Mobile `390x844` side-by-side gegen die jeweils kanonische HTML-Referenz geprüft. Erforderlich sind eine nummerierte Abweichungsliste ohne P0/P1, echte Klickpfade im isolierten Real-/Test-Tenant, klar synthetische Testdaten sowie shrink-only entfernte Alt-Shell-/Navigationsausnahmen. Ohne diese Belege gibt es kein S4/UI-fertig, UX-PASS, Search-Merge oder Production-Live-Erfolg.

**Zweck:** Innenmodul-, Backend- und Gesamtoberflächenstatus werden nicht mehr vermischt; die nächste Produktarbeit schließt zuerst den tatsächlich sichtbaren Nutzerweg.

**Wesentlicher Nachteil:** Bereits grüne modulare Teilkandidaten müssen nach der Shell-Konvergenz erneut als vollständige Route abgenommen werden; diese bewusste Wiederholungsarbeit verhindert, dass eine technisch grüne Integration in einer verworfenen Oberfläche als Produktlieferung gilt.

## D-UI-CORE-002 — Auslegungsfreie Ziel-Shell, Rollenwege und Kartenwahrheit (Owner 2026-09-10)

**A — Einstieg und Rollen-Home:** `/start` ist ausschließlich der unauthentifizierte Fundament-Login ohne Wetter-, Event- oder Provider-Platzhalter. Nach erfolgreicher PIN führt der Weg auf `/`. `/` ist das eine rollenbewusste Home: `werkstatt` erhält Phillip V4; `buero|meister` erhalten Rolf V8; `readonly` erhält Rolf ohne Schreibaktionen; `admin|developer` werden nach `/settings` geführt. Zulässig sind exakt die Rollen `developer|admin|meister|buero|werkstatt|readonly`, keine Aliasrollen. Alle dürfen die akzeptierten Kernbereiche lesen; Mutationen bleiben ausschließlich den bestehenden Command- und Rollenverträgen unterworfen.

**B — Ziel-Shell und Navigation:** Die Shell folgt der Rolf-V8-Machart. Sichtbare Hauptziele sind ausschließlich Home/Der Tag, Werkstatt, Aufträge, Kunden & Kontakt, Geld & Rechnungen als Accounting-minimal sowie berechtigte Einstellungen. Kalender erscheint erst nach realer Microsoft-365-Abnahme. Phillip behält die kompakte Kopfleiste und fünf feste Werkstattaktionen. Tablet/Mobile erhält das Rolf-Dock Der Tag, Aufträge, Kunden, Geld und Mehr, ohne Altmodule. Der Werkstatt-Empty ist ehrlich und bietet eine sinnvolle Primäraktion.

**C — Routendisposition:** `ENTFÄLLT` bedeutet nach Link-/Importprüfung Route entfernen. `QUARANTÄNE` bedeutet keine produktiv rendernde Page und kein Navigationsziel; direkte URLs enden fail-closed/404. Galvanik bleibt nur flacher Blackbox-Drilldown; `station/[slug]` entfällt. Wareneingang und Warenausgang sind Aktionspfade. `archive` wird in Orders absorbiert; `feedback` besitzt keine Produktpage; `status` ist intern/Admin/API; `telefonnotiz` gehört zu Customers/Intake; `items` ist kein eigenes Modul; `start` bleibt ausdrücklich Auth-Einstieg.

**D — Orders und Customers:** Listen sind ausschließlich reale Such- und Filtereinstiege. `/orders/[id]` rendert dieselbe verbindliche V8-Auftragskarte, `/customers/[id]` dieselbe V2-Kundenkarte. Es gibt genau eine Overlay-/Backstack-Wahrheit für Home→Auftrag→Kunde→Auftrag, Kunde→Auftrag und Intake→Auftrag. Rechnung und Zahlung bleiben in der Auftragskarte. Filter und Scrollposition bleiben beim Zurückgehen oder Schließen erhalten. `NOT_AVAILABLE`- und „kommt bald“-Flächen sind unzulässig.

**E — Lieferreihenfolge und Gate:** Nach dem Governance-Paket steuert die Mission exakt ein Programm `PATH1_UI_CONVERGENCE`; dessen erster interner Schritt ist A Shell/Rollen, danach folgen B Orders V8, C Customers V2, D Suche auf der Ziel-Shell erneut integrieren und abnehmen sowie E Kalender. A wird mit realen Logins aller sechs Rollen sowie vollständigen authentifizierten Routen bei `1914x917`, `1220x880` und `390x844` side-by-side abgenommen. Alle sichtbaren Navigationsziele werden geklickt, entfernte und quarantinisierte Direkt-URLs negativ geprüft, Alt-Shell-/Navigationsausnahmen shrink-only reduziert und die nummerierte Abweichungsliste enthält keinen P0/P1. PR #84 bleibt bis nach A bis C `DRAFT/CANDIDATE` und wird nicht in die Alt-Shell gemergt. F1.6 und Kalender bleiben nicht gestartet.

**Owner-Nachschärfung 2026-09-10 — kanonischer Vollersatz:** Die bestehende sichtbare Oberfläche ist vollständig als Lieferbasis verworfen. Alte Shell, Navigation, Startseite, Orders-/Customers-Oberflächen und responsive Navigation werden vollständig ersetzt; sie werden weder umgestylt noch umsortiert, einzeln repariert oder schrittweise als Ziel-UI weiterentwickelt. Wiederverwendbar sind ausschließlich belastbare Backend-, Daten- und Auth-Verträge sowie technisch sinnvolle Logik hinter der Oberfläche. Die alte Informationsarchitektur und Darstellung sind keine Vorlage.

Das eine kanonische Programm `PATH1_UI_CONVERGENCE` verbindet exakt die vier freigegebenen Referenzen `KREILE_STARTSEITE_PHILLIP_V4_2026-08-20.html`, `KREILE_STARTSEITE_ROLF_V8_2026-08-20.html`, `KREILE_AUFTRAGSKARTE_MACHART_V8_2026-08-19.html` und `KREILE_KUNDENKARTE_MACHART_V2_2026-08-19.html` rollenrichtig mit Navigation und Rückwegen. Seine internen Etappen sind A Shell/Rolleneinstieg, B Orders V8, C Customers V2, D Suche auf der neuen Oberfläche und E M365-Kalender. Technische Zwischenstände sind zulässig, aber kein einzelner Teil-, Kosmetik- oder Reparaturpatch ist UX-Lieferfortschritt, Zielscreen-PASS oder eine livefähige Oberfläche. Akzeptanz entsteht erst für die zusammenhängende Oberfläche auf Desktop, Tablet und Mobile. HTML-Demodaten bleiben ausschließlich visuelle Referenz und werden weder Produktdaten noch Fake-Fallback; Nebenmodule, Platzhalterseiten und Schattenmodule sind unzulässig.
