# F0_MODULAR_FOUNDATION_MAP â€” Foundation-Owner, Clients, Importregeln (F0-07)
...
- **Befund (Grenzverletzung):** Direkte `@supabase/supabase-js`-`createClient(url, serviceKey)`-Instanzen
  ...
  und CI-Regel gegen direkte `createClient`-Aufrufe auÃŸerhalb `lib/supabase/*`.

## Importregeln (Zielvertrag)
1. Modulgrenzen nur Ã¼ber Ã¶ffentliche TS-Typen, Ports/Provider, Component-Props, versionierte SQL-Views/Functions.
...
- Modulmanifest-Vertrag (Modul-ID, Version, Owner, Ã¶ffentliche Exporte, Capabilities, Migrationen,
  Views/Functions, Storage-Zwecke, Events, Config, AbhÃ¤ngigkeiten) als validiertes Schema.
