...
Produkt-SHA des Browserlaufs: `4faa2ba97c0a4b6a7c43d7a3fcc06bdd7ce64609`
...
7. 55 laut Kanon entfallene oder nicht rendernde Quarantäne-Routen liefern echte 404-Antworten. `/warendurchlauf/galvanik` bleibt als flacher, realer Blackbox-Drilldown erhalten.
...
- Klickpfade: Rolf → Direktaufnahme → Auftrag; Orders → Auftrag → Kunde → Auftrag → zurück; direkte V8-/V2-Deep-Links; Suche → V8/V2; Rolf → Geld & Rechnungen; Gregor → E-Mail-Login → Einstellungen → Start → Einstellungen; Phillip-Home.
- Viewports: `1914x917`, `1220x880`, `768x1024`, `390x844`.
- Alle 20 PNGs sind neu auf dem Produkt-SHA erzeugt, vollständig gerendert, nicht leer und hashgebunden. Die gespeicherten Pixelmaße entsprechen dem zweifachen Device-Pixel-Ratio der logischen Viewports.
- P3-Browservertrag: 1/1 PASS in 1,3 Minuten, keine feste Wartezeit, keine Dev-Overlays, strikte Fehlerliste der aktiven Zieloberflächen leer.
- Negative Routematrix: 55/55 echte 404; die erwarteten 404-Konsoleffekte laufen auf einer getrennten authentifizierten Prüfseite und können echte Zieloberflächenfehler nicht verdecken.
- P2-Kernweg auf demselben Produkt-SHA: 1/1 PASS in 35,2 Sekunden; Kunde → KV → genau ein bestehender F1.1-Auftrag einschließlich Reload, Readback und Datumsreset.
- Receipt: `docs/evidence/path1/artifacts/p3-core-surfaces-search/p3-browser-receipt.json`
- Receipt-SHA-256: `0d590203631668711ee501cf40552ea48fe567fbf53e7e4f0ced23d2f906b069`
- Das Receipt bindet Produkt-SHA, Tenant, drei Actor-IDs, 55 Route-Ergebnisse sowie Dateiname, Zustand, Viewport und SHA-256 aller 20 Bilder.

## Lokale Abnahme

| Gate | Ergebnis |
| --- | --- |
| Erweiterte Fokustests | PASS — 22 Dateien, 277/277 Tests |
| Vollständige Unit-Suite | PASS — 112 Dateien, 926/926 Tests |
...
| W4-Anker | PASS — unveränderter Test-Blob `923988f0bd489ccb0730e5b08b6e4066f3cb6283` und bestehender Realvertrag grün |
| F1.4/F1.5 Fresh-DB | PASS — 5 Dateien, 16/16 Tests inklusive Storno-/Zahlungskonkurrenz |
| F1.3 Live Card Fresh-DB | PASS — 9/9 Tests |
...
| Production-Build | PASS — Next.js 16.2.12, 31 App-Ausgaben; entfallene Routen nicht gebaut |
| Diff-/Scope-Check | PASS — Produktcommit ohne vorzeitige Evidence; Evidence separat gebunden |
