...
/** F1.4 ratifies exactly one tax rate and exactly one payment term. */
const RATIFIED_VAT_RATE_BASIS_POINTS = 1900;
const RATIFIED_PAYMENT_TERM_DAYS = 14;
...
type InvoiceRow = {
  ...
  snapshot_vat_rate_basis_points: number;
  snapshot_payment_term_days: number;
  due_date: string;
  /** `(issued_at AT TIME ZONE 'Europe/Berlin')::date + 14`, read from the DB. */
...
/**
 * Parst exakt die heruntergeladenen PDF-Bytes mit der vorhandenen
 * pdfjs-Testabhaengigkeit und gibt die Textebene in Lesereihenfolge zurueck.
 * Kein Fixture, kein Buffer.includes, kein React-Baum: bewiesen wird nur, was
 * im echten Byte-Strom als Text steht.
 */
async function readPdfText(bytes: Buffer): Promise<{ spaced: string; compact: string }> {
  ...
  try {
    ...
    const spaced = fragments.join(" ").replace(/\s+/g, " ").trim();
    expect(spaced.length).toBeGreaterThan(200);
...
/** Jeder `N %`-Token des Dokuments, in Lesereihenfolge. */
function vatRateTokens(spacedText: string): string[] {
  return [...spacedText.matchAll(/(\d+(?:,\d+)?) ?%/g)].map((match) => match[1]);
}

/** `YYYY-MM-DD` als deutscher Kalendertag `DD.MM.YYYY`, rein lexikalisch. */
function germanCalendarDay(isoDate: string): string {
    ...
    try {
      ...
      const [issuedRow] = await sql<InvoiceRow[]>`
        SELECT id::text, invoice_number, status, aggregate_version, order_version,
               ...
               gross_amount_cents, service_date::text, payment_term_days,
               (snapshot->'totals'->>'vatRateBasisPoints')::integer
                 AS snapshot_vat_rate_basis_points,
               (snapshot->>'paymentTermDays')::integer AS snapshot_payment_term_days,
               to_char(due_date, 'YYYY-MM-DD') AS due_date,
               to_char((issued_at AT TIME ZONE 'Europe/Berlin')::date + 14, 'YYYY-MM-DD')
                 AS berlin_due_date,
               pdf_sha256, pdf_content,
      ...
      expect(issuedRow).toMatchObject({
        ...
        vat_rate_basis_points: RATIFIED_VAT_RATE_BASIS_POINTS,
        vat_amount_cents: 1900,
        gross_amount_cents: 11900,
        service_date: serviceDate,
        // Der ratifizierte Vertrag: Zeile, Snapshot und Berlin-Faelligkeit.
        payment_term_days: RATIFIED_PAYMENT_TERM_DAYS,
        snapshot_vat_rate_basis_points: RATIFIED_VAT_RATE_BASIS_POINTS,
        snapshot_payment_term_days: RATIFIED_PAYMENT_TERM_DAYS,
      ...
      const dueDate = issuedRow?.due_date ?? "";
      // Die Faelligkeit ist der Berliner Ausstellungstag + genau 14 Tage.
      expect(dueDate).toBe(issuedRow?.berlin_due_date);
      const germanDueDate = germanCalendarDay(dueDate);

      // Echte heruntergeladene Bytes, mit pdfjs geparst: 19 % und 14 Tage
      // stehen im Dokument, 7 % und 30 Tage kommen darin nicht vor.
      const originalPdfText = await readPdfText(originalPdf);
      expect(originalPdfText.compact).toContain("zzgl.USt(19%)");
      expect(originalPdfText.compact).toMatch(new RegExp(
        `${escapeRegExp("Zahlungsziel:14Tagenetto,")}[^]{0,16}${escapeRegExp(germanDueDate)}`,
      ));
      expect(originalPdfText.compact).toContain(`Rechnungsnummer:${firstInvoiceNumber}`);
      expect([...new Set(vatRateTokens(originalPdfText.spaced))]).toEqual(["19"]);
      expect(originalPdfText.compact).not.toContain("USt(7%)");
      expect(originalPdfText.compact).not.toContain("30Tagenetto");
      ...
      // Das unveraenderte Original traegt nach dem Storno exakt denselben Text.
      const originalAfterCancelText = await readPdfText(originalAfterCancel);
      expect(originalAfterCancelText.compact).toBe(originalPdfText.compact);

      // Der echte Stornobeleg: 19 %, kein Zahlungsziel, kein 7 % und keine 30 Tage.
      const cancellationPdfText = await readPdfText(cancellationPdf);
      expect(cancellationPdfText.compact).toContain("StornoUSt(19%)");
      expect(cancellationPdfText.compact).toContain(`ZuRechnung:${firstInvoiceNumber}`);
      expect([...new Set(vatRateTokens(cancellationPdfText.spaced))]).toEqual(["19"]);
      expect(cancellationPdfText.compact).not.toContain("USt(7%)");
      expect(cancellationPdfText.compact).not.toContain("30Tagenetto");
      expect(cancellationPdfText.compact).not.toContain("Zahlungsziel");
      ...
      expect(cancelledRow).toMatchObject({
        ...
        aggregate_version: 2,
        order_version: 3,
        // Das Storno aendert weder Steuersatz noch Zahlungsziel noch Faelligkeit.
        vat_rate_basis_points: RATIFIED_VAT_RATE_BASIS_POINTS,
        payment_term_days: RATIFIED_PAYMENT_TERM_DAYS,
        snapshot_vat_rate_basis_points: RATIFIED_VAT_RATE_BASIS_POINTS,
        snapshot_payment_term_days: RATIFIED_PAYMENT_TERM_DAYS,
        due_date: dueDate,
        berlin_due_date: dueDate,
        pdf_sha256: originalPdfSha256,
      ...
      const finalInvoices = await sql<{
        ...
        vat_rate_basis_points: number;
        payment_term_days: number;
        due_date_is_berlin_plus_14: boolean;
      }[]>`
        SELECT id::text, invoice_number, status, pdf_sha256,
               vat_rate_basis_points, payment_term_days,
               due_date = (issued_at AT TIME ZONE 'Europe/Berlin')::date + 14
                 AS due_date_is_berlin_plus_14
        FROM public.invoices
        WHERE tenant_id = ${TENANT} AND order_id = ${orderId}
      ...
      // Jede F1.4-Rechnung dieses Auftrags traegt exakt den ratifizierten Vertrag.
      expect(finalInvoices.map((invoice) => ({
        vat: invoice.vat_rate_basis_points,
        term: invoice.payment_term_days,
        berlinDue: invoice.due_date_is_berlin_plus_14,
      }))).toEqual([
        { vat: RATIFIED_VAT_RATE_BASIS_POINTS, term: RATIFIED_PAYMENT_TERM_DAYS, berlinDue: true },
        { vat: RATIFIED_VAT_RATE_BASIS_POINTS, term: RATIFIED_PAYMENT_TERM_DAYS, berlinDue: true },
      ]);
      ...
      writeFileSync(
        ...
        `${JSON.stringify({
          ...
          serviceDate,
          ratifiedContract: {
            vatRateBasisPoints: RATIFIED_VAT_RATE_BASIS_POINTS,
            paymentTermDays: RATIFIED_PAYMENT_TERM_DAYS,
            dueDate,
            dueDateSource: "berlin_issue_day_plus_14",
            pdfTextProof: "pdfjs-parsed downloaded bytes: 19 % and 'Zahlungsziel: 14 Tage netto' present, 7 % and 30 days absent",
          },
          firstInvoice: {
            id: firstInvoiceId,
